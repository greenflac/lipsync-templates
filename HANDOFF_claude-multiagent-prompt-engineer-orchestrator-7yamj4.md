...
## 2026-08-29 — знание подключено, поиск научился читать по-русски, и три моих ошибки

ПОМЕТКИ НАВЕРХ (Ц4). Лицензия `sentence-transformers` — НЕПРОВЕРЕНО. dense-канал
по-прежнему выключен: `build_report['dense'] = 'could not measure'`.
31 находка критиков (22 продукт/архитектура + 9 схема) НЕ вложена — не считать
решения готовыми.

СОСТОЯНИЕ, ИЗМЕРЕНО 2026-08-29:
индекс 5216 = core 12 + gallery 4601 + community 473 + knowledge 130.
918 утверждений о 231 модели. 42 из 42 семейств вендоров достижимы.
recall@5 = 0.4667 против 0.0167 случайного.
Генераций прогнано: 0. Оценок: 0.

### Мультиагентный заход: GenAI-инженер с дизайнерской насмотренностью

13 агентов, 5 фаз, ~104 минуты, 0 ошибок. Прибор строился ДО сбора (П1).

ДИАГНОЗ, поставленный прибором: из 5086 записей корпуса 5074 — примеры промтов,
12 — правила про один случай. Слова `key light`, `bokeh` встречаются 25-199 раз,
но ТОЛЬКО внутри текстов промтов, как токены. Словарь был, знания не было.

База по 36 живым вопросам: годно 1, не годно 22, не смогли 13. У набора свой
негативный контроль: 5 пар из model_facts.jsonl вернули pass, значит «не смогли»
на остальных — дыра, а не сломанный прибор.

Собрано 130 записей: фундамент 34, вендор 41, comfyui 18, ремесло 37.
105 уникальных URL. `wc -l` сошлось с заявленным у всех четырёх — приписок нет.
Проверено вручную: запись про рембрандтовский свет (40-60° от оси, 30-45° выше)
подтвердилась строкой 26 указанной страницы.
Вердикт по стоп-условию: 23 из 36. Токены `must_cite` найдены у 21 из 21
вопроса с опознаваемыми токенами — проверял грепом независимо.

Критик схемы записи: НЕ ГОДНО, 9 нарушений из 12, каждое с прогоном.

### Три дефекта, найденных критиком и починенных

1. ТОКЕНИЗАТОР НЕ ЧИТАЛ КИРИЛЛИЦУ. `query_terms('покажи промт...')` -> [],
   `retrieve` -> fail «искал 5074, ничего не подошло». Это не отсутствие фичи, а
   неправда: вердикт о запросе, который не смогли прочитать. Операторы пишут
   по-русски.
2. ПУСТОЙ ЗАПРОС получал такой же уверенный `fail`. Теперь `не смогли`, checked 0.
3. КВОТА ДУШИЛА ЗНАНИЕ: 300 записей под одним провенансом -> retrieve(k=8)
   вернул 2. Отдельная квота 6 (ВЫБРАНО). Плюс `vendor:comfyui` падал на
   фолбэк 0.5 — НИЖЕ галереи, которую должен перебивать.

130 записей подключены отдельной полосой и ВОЗВРАЩАЮТСЯ РАЗДЕЛЬНО. Намерение не
угадывается сознательно: роутер без механизма — это пожелание, а этому пришлось
бы решать по маркерам на двух языках. `examples` — промты, `knowledge` — второй
помеченный список.

ИЗМЕРЕНО, полоса выкл -> вкл:
    'почему кожа пластиковая на портрете'     fail 0 -> pass 5
    'какой параметр отвечает за силу промта'  fail 0 -> pass 5
    'warm golden hour amber palette'          pass 5 -> pass 5  (регрессии нет)

recall@5 НЕ ИЗМЕНИЛСЯ — 0.4667. Это честный результат: тот набор мерит поиск по
промтам, полоса знания его не трогает. Сказать, что вырос, было бы враньём о том,
какой прибор читали.

Мутации в обе стороны: кириллица убрана -> 4 теста красные; квота знания = 2 ->
красный; = 99 -> красный; вес знания ниже галереи -> красный.

### ТРИ МОИХ ОШИБКИ ЗА ЭТОТ ЗАХОД

1. ЗАПУШИЛ КРАСНЫЙ ГЕЙТ. Сцепил прогон, коммит и пуш в одну команду; `echo` кода
   возврата съел статус, и пуш проехал поверх красного. Гейт сработал — я его
   обезоружил. Дисциплина: код возврата гейта снимается ОТДЕЛЬНОЙ командой.
2. ГЕЙТ, ТРЕБУЮЩИЙ НЕЗАКОММИЧЕННЫЙ ФАЙЛ. `check_craft.py` считал `file://` в
   отсутствующий файл нарушением. Здесь зелено, в CI красно: корпус намеренно не
   коммитится. Указатель был верный, файла не было НАРОЧНО — третий исход, и я
   свернул его во второй в том самом гейте, где сам это правило и записал.
3. ТЕСТ, ЧИТАЮЩИЙ ТО, ЧТО ЛЕЖИТ НА МАШИНЕ — в тестах, которые эту болезнь и
   документируют. Строили настоящий индекс: здесь 5074 промта, в CI ноль.

ТРЕТИЙ СЛУЧАЙ ЭТОГО КЛАССА ЗА ДВОЕ СУТОК (до него: `KLING_KEY` протекал в probe-
тесты; `load_craft_records` по умолчанию возвращал отключённый источник).
Поэтому починено не по месту, а по форме (И7):

* `studio/mcp/tests/test_no_test_needs_the_uncommitted_corpus.py` — обход AST
  всех тестов, валит любой `build_index()` без указания корпуса. ИЗМЕРЕНО: 12
  вызовов, 12 явных. Список исключений пуст НАМЕРЕННО. Оба контроля на месте:
  ноль найденных вызовов = сломанный обход (Р2); негативный контроль различает
  голый вызов и явный (И5).
* `scripts/check_as_ci` — гейт в отделённом worktree на HEAD, то есть ровно на
  том, что видит CI. Рабочая копия не трогается: ручной вариант (унести корпуса,
  прогнать, вернуть) оставляет репозиторий сломанным, если прогон умрёт.
  Негативный контроль настоящий: коммитил дефект -> EXIT=1 с названной причиной,
  откатывал -> EXIT=0.
  ОГОВОРКА: проверяет HEAD, не рабочую копию. Порядок — коммит, прогон, пуш.

### План релиза

`docs/` его не содержит намеренно — он отдан владельцу отдельным документом.
Четыре этапа: (1) замкнуть петлю обратной связи — session_events и запись
результата генерации; (2) долечить поиск — 9 находок критика схемы, включить
dense; (3) развязать корпус и деплой; (4) релиз-кандидат.
Гейт релиза: зелёная сборка И слепой контроль без нарушений И 36 из 36 вопросов
отвечены со ссылками И хотя бы одно продуктовое число из настоящих данных.

ЖДЁТ ВЛАДЕЛЬЦА, три решения: хранилище корпуса (сейчас не едет вместе с кодом);
$1.05 на счёт Kling (замер одобрен, вернулось `429 code 1102`); драйвинг-клипы
под lipsync (присланные танцевальные, лицо к камере на всём окне только у одного).

НЕ ОБЪЯВЛЯТЬ РЕЛИЗ ПО ЗЕЛЁНОМУ ГЕЙТУ. Гейт говорит, что код делает написанное.
Он не говорит, что кто-то получил видео, которое захотел опубликовать.

## 2026-08-30 — агент-валидатор: стенд построен, первый прогон состоялся

Владелец дал 5 часов и разрешил решать самостоятельно по методологии.

### Решения, принятые без владельца

1. **OpenFake БЕРЁМ.** Сначала я решил исключить его по лицензии; владелец
   поправил, и справедливо: Ц5 говорит «помечать отдельно», а не «не брать».
   Пометка сделана СТРУКТУРНОЙ: счётчик делит две популяции, а вердикт печатает
   маркер сверху, где его нельзя пролистать.
2. **Читатель — свежий субагент, а не я.** Слепота по договорённости не слепота.
   Агент, которому дали путь к файлу, физически не может слить то, чего не
   получал (И1).
3. **Детектор вжжённых меток НЕ ставим в гейт** — он не работает, см. ниже.

### Что построено

`studio/mcp/casebank.py` — ядро: чистка картинок и видео, склейка, чтение
удалённого parquet по Range.
`scripts/build_casebank.py` — сборка банка. `scripts/contact_sheets.py` —
раскадровки. `scripts/score_validation.py` — счёт. `scripts/validator.py` —
гейт на утечку. `scripts/check_burned_marks.py` — отрицательный результат.

### Источники, ИЗМЕРЕНО

`kling` (bitmind/klingai-videos, cc-by-4.0): 9001 строка серверного лога задач
самой площадки. Поштучно: prompt, negative_prompt, cfg, duration, camera_json,
kling_version (1.0 — 3028, 1.5 — 4160, 1.6 — 95), внутреннее имя движка.
ПЛЮС ОЦЕНКА ПОЛЬЗОВАТЕЛЯ: like 1736, dislike 118, без оценки 7147, с причинами
(«poor aesthetics» 46, «Text mismatch» 25). Это applicability-данные, которых у
базы нет вообще. И 211 задач `m2v_video_lip_sync` — наш продукт.

`openfake` (cc-by-nc-4.0): 22 генератора в одном шарде, включая veo-3, sora-2,
midjourney-7, nano-banana-pro, gpt-image-2, seedream-v5.0.

### Пять утечек, которые я заложил и поймал

1. **Метаданные картинки.** Граф ComfyUI, 6259 символов. Снимается пережатием.
2. **Теги контейнера видео.** `-map_metadata -1` оставляет `encoder`; нужен
   `-fflags +bitexact`. Поймано контролем с подложенным `title: kling_version 1.6`.
3. **`convert()` и `copy()` протаскивают `info`.** PNG-чанки отваливаются, а
   JPEG-комментарий — нет. Первая чистка выглядела исправной, потому что её
   проверяли на хрупком носителе. Чистым выходит только новый объект Image.
4. **Идентификаторы разборов.** Файлы назывались `kv-…` и `of-…`, по источнику и
   по порядку. Теперь `case-001.jpg`, порядок перемешан фиксированным сидом.
5. **Комментарий раскадровки.** ffmpeg пишет `Lavc61.3.100`. Он называет НАШ
   инструмент, не источник, и выглядел безобидно — пока тот же контроль не
   заметил, что у картинок OpenFake комментария нет ВООБЩЕ. Само его наличие
   отделяло видео-разбор от картиночного, не глядя в кадр. Утечке не нужно
   называть ответ: достаточно с ним коррелировать.

### Шестую нашли читатели, и она была не в метаданных

У роликов Kling **вотермарк вжжён в пиксели** и читается как «KLING AI 1.6» —
модель и версия словами. Первые четыре читателя вернулись с уверенностью 0.95 и
одним и тем же наблюдением. 16 из 40 разборов отвечались чтением подписи.

Это стенд, работающий как задумано: он нашёл изъян раньше, чем опубликовано
хоть одно число, и нашёл его у собственного автора.

Починено обрезкой 12% снизу, ПРОВЕРЕНО ГЛАЗАМИ (П3) — и не зря: первая правка
молча не применилась (полоса осталась 2280×214 после переформатирования), а
руками тот же фильтр работал. Обрезка, которая ничего не делает, выглядит как
работающая.

### ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ (И6): детектор вжжённых меток не вышел

Замысел: наложение — это одинаковые пиксели в одном месте у картинок, которые
больше ничем не связаны. Замерено на банке с ИЗВЕСТНЫМ загрязнением:

    16 помеченных Kling      медиана 0.13  макс 0.47  доля>0.7  0.0%
    24 чистых OpenFake       медиана 0.22  макс 0.78  доля>0.7  2.9%

Загрязнённая группа даёт МЕНЬШЕ чистой. Маленький полупрозрачный логотип —
погрешность округления в статистике по участку, содержимое которого меняется
целиком. В гейт НЕ поставлен: он печатал бы «годно» над заведомо грязным банком.
Что, вероятно, сработает: вычесть медианное изображение группы и смотреть остаток.

### Первый прогон, промежуточно (18 из 40 на момент записи)

    kling      8/8  = 1.00   ← вотермарк, не чтение
    openfake   4/9  = 0.44   ← случайное 0.083, впятеро выше
    вердикт: не смогли — отвечено 17 < 20, процент по такой выборке слух

### Открыто

* Пересчёт Kling на обрезанных раскадровках — стенд готов, прогон не сделан.
* Анализ признаков из поля `observed`: что реально работает, проверка на
  отложенных 412 строках.
* Детектор наложений вторым заходом, если он вообще нужен.

### Первый прогон, ОКОНЧАТЕЛЬНО: 40 из 40 читателей

    разборов 40 | отвечено 36 | не смогли 4
    семейство   22/36 = 0.611   против случайного 0.0833
      kling     16/16 = 1.00    ← вотермарк, не чтение
      openfake   6/20 = 0.30    ← ЧИСТАЯ ПОЛОВИНА, честное число
    модель      12/36 = 0.333
    узнал по памяти: 0

ЧЕСТНЫЙ ЗАГОЛОВОК — 0.30, а не 0.61. Половина Kling отвечалась чтением подписи
«KLING AI 1.6», вжжённой в кадр. На двадцати отвеченных картинках из двенадцати
закрытых генераторов против базы 1 к 12 — это в 3.6 раза выше случайного.

Полная выборка легла НИЖЕ промежуточной (0.357 на 26 → 0.30 на 40) — обычное
направление для числа, которое перестаёт быть везением по мере набора. Записано
падение, а не более раннее значение.

Ни один читатель не отметил узнавание по памяти, поэтому числа с ними и без них
совпадают: это чтение, а не припоминание.

Короткий список признаков по-прежнему возглавляет загрязнение — «полоса»,
«широкая», «кадрами», «углу», «нижнем», «правом». Первое — вотермарк, второе —
раскол видео/картинка, то есть лёгкая половина вопроса. Оба наблюдения верны и
ни одно не является находкой про генераторы.

ГДЕ ЛЕЖИТ РЕЗУЛЬТАТ: `work/casebank/` — TRUTH.json, ANSWERS.json, SCORE.json,
BLIND_MAP.json и 62 файла медиа. Каталог в .gitignore, поэтому числа продублированы
здесь: при пересборке в новой сессии банк собирается заново командой
`python scripts/build_casebank.py --kling 16 --openfake 24` (сид фиксирован,
выборка воспроизводится).

ИДЁТ НА МОМЕНТ ЗАПИСИ: пересчёт 16 роликов Kling на ОБРЕЗАННЫХ раскадровках
(wf_39ae50ba-1a2). Он даст второе честное число — про видео. Обрезка проверена
глазами: полоса 2280×188 вместо 2280×214, подписи нет.

---

## Пересчёт на обрезанных раскадровках: вотермарк сняли, а число не сдвинулось — и это находка

16 из 16 читателей вернулись (wf_39ae50ba-1a2). Ответы наложены на исходные
`scripts/merge_answers.py` (новый файл: накладка по case_id, ид, не легший
никуда, печатается, а не выбрасывается — иначе знаменатель тихо съезжает вниз).
    ...
    основа 40 | накладок 16 | вышло 40
    заменено 16 | не легло никуда 0

Проверено, что заменённые 16 — ровно kling: `Counter({'kling': 16})`.

### Что изменилось у читателей

- Уверенность на Kling: было 0.95 (читали подпись буква за буквой), стало 0.3–0.45.
- «Не смогли» на Kling: было 0, стало 3 из 16.
- Формулировки: «полоса из 6 кадров = видео, значит из кандидатов остаются
  kling / veo-3 / sora-2 / wan», дальше — распределительный приор, а не признак.

### Что НЕ изменилось, и почему это дефект прибора, а не успех агента

Семейство на Kling осталось 13/13 = 1.00. Не потому, что читатели научились
видеть Kling, а потому, что **все видео в банке — Kling**. Форма раскадровки
(широкая полоса из 6 кадров) объявляет источник; дальше достаточно назвать
самый вероятный видео-движок, и ответ всегда верен. Входа, на котором
правильный ответ «не kling», в банке НЕТ — это отсутствующий негативный
контроль (И5), а процент без него не измеряет различение.

Так что вотермарк был не единственной подпоркой: под ним лежала вторая, того же
рода. Обрезка сняла первую и обнажила вторую.

### Прибор исправлен, а не отчёт

`scripts/score_validation.py` — пятый способ соврать: источник с единственным
семейством помечается `различимо: false`, его процент печатается, но НЕ выносится
в заголовок; вердикт считается по различающей части.

МУТАЦИЯ (Т1), константа `len(f) < 2`, обе стороны, `__pycache__` вычищен:

    len(f) < 1  → FAIL ×2 (test_a_single_family_source_is_marked…,
                           test_two_versions_of_one_vendor_are_still_one_family)
    len(f) < 3  → FAIL ×1 (test_a_bank_where_every_source_discriminates_names_none)
    восстановлено → Ran 11 tests, OK

Негативный контроль есть с обеих сторон: банк, где все источники различают
(флаг обязан молчать), и банк из двух ВЕРСИЙ одного вендора — там флаг обязан
сработать, потому что семейство одно. Второй тест поставлен нарочно, чтобы
никто не «починил» банк дешёвым способом: добавить kling 2.0 не помогает,
нужен ВТОРОЙ ВЕНДОР видео.

### ЧЕСТНОЕ ЧИСЛО ПОСЛЕ ИСПРАВЛЕНИЯ

    разборов 40 | отвечено 33 | не смогли 7
    ИТОГ (различающая часть): семейство 6/20 = 0.30, модель 5/20 = 0.25
    всё вместе, для справки:  семейство 19/33 = 0.5758, модель 7/33 = 0.2121
    случайное при 12 семействах = 0.0833
      kling     13/16 отвечено, семейство 1.0  ← одно семейство, НЕ ИЗМЕРЕНИЕ
      openfake  20/24 отвечено, семейство 0.30
    узнал по памяти: 0
    pass: семейство 0.30 против случайного 0.0833

**0.30 — это то же число, что было до пересчёта.** Второго честного числа
(про видео) пересчёт НЕ дал: он показал, что его пока и нельзя получить.
NON-COMMERCIAL: вся различающая часть — OpenFake, cc-by-nc-4.0. Пометка идёт
дальше с любым выводом отсюда.

### Отрицательный результат, записанный числом (И6)

Гипотеза «уберём вотермарк — увидим, умеет ли агент читать видео» проверена и
НЕ подтвердилась: 16 роликов, 13 ответов, 1.00 — величина, которую нельзя
интерпретировать. Стоимость: один прогон из 16 агентов, 482 с, 740k токенов.

### Что нужно, чтобы измерить видео (владельцу)

Второй видео-вендор с известным происхождением, ≥16 роликов: Veo, Sora, Wan или
Runway — серверный лог или собственная генерация, не подпись загрузчика.
Пока его нет, любое число про видео у нас — не измерение, и так и напечатано.

---

## Civitai как источник видео: количество есть, грейд истины другой (ИЗМЕРЕНО)

Владелец: «на civitai есть ролики на любой вкус и модель». Проверено командой,
а не рассуждением — `scripts/probe_civitai_video.py`, 3 страницы Most Downloaded
за месяц, 60 версий с видео:

    роликов просмотрено                 191
    с полем модели в СВОИХ метаданных    11  (5.8%)
    истина = подпись страницы           180  (94.2%)
    ключи: resources 6, civitaiResources 5, workflow 5
...
ХОРОШАЯ НОВОСТЬ: не-Kling видео там есть и в количестве. Wan прямо в списке
кандидатов, MiniMax H3 — закрытый вендор. Дефицита материала нет.

ПЛОХАЯ: 94% роликов помечены словом загрузчика, а не записью исполнения. У Kling
истина была серверным логом задач площадки. Это разные грейды, и складывать их
в один процент молча — ровно та ошибка, которую сегодня уже дважды ловили.

ВАЖНАЯ ОГОВОРКА В ПОЛЬЗУ CIVITAI: негативному контролю ИДЕАЛЬНАЯ метка не нужна.
Он ловит ровно одно — «читатель всегда говорит kling и всегда прав». Это ломает
даже шумная метка. Шумной становится ВЕЛИЧИНА процента, а не сам факт различения.

Решение владельца: принимать ли грейд `uploader_claim` в банк с отдельным учётом.

---

## Владелец разрешил грейд «слово загрузчика» (2026-08-31). Что построено

### Новый источник в банке

`casebank.SOURCES["civitai"]` — `truth_grade: uploader_claim`, `commercial_ok: False`
(Civitai ToS 6.1 — personal, non-commercial; сбор по авторизации владельца
2026-08-27, уже записанной в `studio/knowledge/PROVENANCE.md`). Лицензия
прочитана ДО встраивания (Ц5).

Введено понятие ГРЕЙДА ИСТИНЫ — три значения, и это не тир из лестницы фактов,
а ответ на один вопрос: записала машина или сказал человек.

    vendor_log        серверная запись задачи площадки (Kling)
    authors_pipeline  авторы датасета сгенерировали сами через API (OpenFake)
    uploader_claim    человек выложил файл и вписал модель (Civitai)

### Фильтр, без которого метка была бы мусором

Ролик, висящий на странице КАРТИНОЧНОГО чекпойнта (Illustrious, Pony), сделан
каким-то видеодвижком, которого страница не называет. Её `baseModel` — не метка,
а уверенный ответ не на тот вопрос. ИЗМЕРЕНО: таких 67 из 191 в пробе.
`CIVITAI_VIDEO_BASES` пропускает только видео-базы.

Собрано 16 роликов: wan 8, minimax 6, ltxv 2. Банк стал 56 разборов —
16 Kling + 16 Civitai + 24 OpenFake.

### Утечка, пойманная ДО того, как побежали читатели

`scripts/check_sheet_shape.py` — новый контроль: сортирует ли ФОРМА файла
разборы по источнику, без единого прочитанного пикселя. Первый же прогон:

    высота полосы    сортирует  56/56 = 1.0   ← ПРОТЕКАЕТ
    размер полосы    сортирует  56/56 = 1.0   ← ПРОТЕКАЕТ
    пропорция        сортирует  54/56 = 0.96  ← ПРОТЕКАЕТ

Причина: ширина кадра была фиксированной, высота шла за пропорцией. У Kling один
домашний формат, у Civitai — что отрендерил загрузчик. Третья утечка того же
класса за три дня: не картинка, но коррелирует с источником.

ПОЧИНЕНО В ДВУХ МЕСТАХ:
1. `contact_sheets.FRAME_SIDE = 214` — каждый кадр обрезается по центру в
   КВАДРАТ и масштабируется, все полосы теперь ровно 1284×214. Проверено:
   `{(1284, 214): 32}`. Квадратный кроп выбран как наименее разрушительная
   нормализация: кроп 16:9 срезает вертикальному ролику голову, а паддинг до
   общего холста переносит ту же подсказку из заголовка файла в чёрные поля.
2. Контроль сравнивает ИСТОЧНИКИ ВНУТРИ ОДНОЙ СРЕДЫ. Что файл — полоса, а не
   картинка, читатель видит глазами; это содержимое, а не утечка. Бесплатным не
   должно быть отличие одного ВИДЕО-источника от другого.

После починки: `video: высота полосы 0/32 = 0.0`, все признаки 0.0.
`image` не сравнивался — там один источник, и это «не смогли», а не утечка.

### Слепые копии стали скриптом, а не ручной работой

`scripts/blind_bank.py`. В прошлый раз слепота делалась руками, и карта осталась
файлом, который нельзя воспроизвести. Снимает: имя, порядок (сид), расширение
(всё `.jpg`), метаданные (перепроверка последней возможности поймать носитель).

### Гейт

`check_sheet_shape` вшит в `scripts/check`: красный только на РЕАЛЬНОЙ утечке;
при отсутствии банка на диске (в CI его нет — gitignore) ответ «не смогли», и
это не провал сборки.

### Мутации (Т1), `__pycache__` вычищен, все в обе стороны

    TOLERATED_UNIQUE 0.25 → 0.05  FAIL x1 ; → 0.9  FAIL x1
    «двух источников в среде» → одного          FAIL x3
    UNVERIFIED_GRADE "uploader_claim" → "vendor_log"  FAIL x3
    убрать "minimax" из таблицы семейств         FAIL x2
    убрать "ltx"                                 FAIL x1
    убрать перемешивание в blind_bank            FAIL x2
    падать на .mp4 при отсутствии раскадровки    FAIL x2
    восстановлено → 16 + 9 + 8 тестов, OK

### Дефект в собственном сообщении, пойманный на прогоне

Предупреждение о non-commercial печатало лицензию OpenFake поверх шестнадцати
клипов Civitai — жёстко зашитая константа вместо чтения из кейсов. По Е2
сообщение обязано следовать из того, что реально в банке. Исправлено: лицензии
собираются с разборов и печатаются списком.

### Глазами (П3)

Просмотрены все 32 раскадровки, сведённые в две простыни. Вотермарка Kling нет
нигде. У одного клипа Civitai в первом кадре своя титульная надпись — это
содержимое ролика, а не системная метка: у остальных пятнадцати её нет.
Замечено и записано: у Civitai уклон в портрет и «арт», у Kling — в сток-сцену.
Это различие СОДЕРЖАНИЯ, снять его без ручной курации нельзя, и на него читатель
может опереться. Ограничение честное, в отчёт.

---

## ПЕРВОЕ ЧИСЛО, В КОТОРОМ УЧАСТВУЕТ ВИДЕО (56 разборов, 56 свежих читателей)

    разборов 56 | отвечено 35 | не смогли 21
    ИТОГ  семейство 9/35 = 0.2571   модель 3/35 = 0.0857
    случайное при 14 семействах = 0.0714     → в 3.6 раза выше случайного

      civitai   отвечено  9/16  семейство 0.1111
      kling     отвечено  9/16  семейство 0.4444
      openfake  отвечено 17/24  семейство 0.2353

    по грейду истины:
      записана машиной    26/40 отвечено, семейство 0.3077
      написана человеком   9/16 отвечено, семейство 0.1111
    узнал по памяти: 0

NON-COMMERCIAL: 40 из 56 (OpenFake + Civitai). Истину написал человек: 16 из 56.
Обе пометки печатает прибор сам, над числом.

### Главное: подпорка обвалилась ровно там, где предсказано

    Kling с вотермарком, банк только из Kling      16/16 = 1.00
    Kling обрезан,       банк только из Kling      13/13 = 1.00   ← не измерение
    Kling обрезан,       в банке три видео-семейства  4/9 = 0.4444 ← ПЕРВОЕ ЧЕСТНОЕ

Гипотеза «единица держится на том, что другого видео в банке нет» проверена
и ПОДТВЕРДИЛАСЬ. Как только у читателя появился выбор среди видео, Kling упал
с 1.00 до 0.44. Ради этого числа и брали Civitai.

### Прибор занижал сам себя, и это тоже исправлено

Первая версия правила И5 спрашивала, сколько семейств в ИСТОЧНИКЕ. Пока Kling
был единственным видео, это работало. С приходом Civitai стало неверно: у Kling
по-прежнему одно семейство, но читатель, глядя на полосу, теперь выбирает из
трёх, и эти ответы РАЗЛИЧАЮТ. ИЗМЕРЕНО: правило по источнику выбрасывало девять
отвеченных разборов Kling с 0.4444, то есть ровно те данные, ради которых всё
и делалось.

Правило переписано на СРЕДУ: источник «не различает», когда в его среде меньше
двух семейств — потому что среду читатель определяет бесплатно, просто посмотрев.
Итог перестал считаться с изъятиями: различающая часть = все 35 ответов.

Мутации (Т1), `__pycache__` вычищен:
    среда → обратно источник      FAIL x1 (ровно тот тест, ради которого правка)
    порог среды < 2 → < 3         FAIL x4
    восстановлено → Ran 17 tests, OK

### Честные оговорки

1. «Не смогли» выросло с 7/40 (17.5%) до 21/56 (37.5%). Банк стал ТРУДНЕЕ, и
   отказ ничего не стоит — читатели им пользуются. Это признак работающего
   прибора, а не поломки.
2. OpenFake 0.30 → 0.2353 сравнивать НЕЛЬЗЯ как падение: 6/20 против 4/17,
   разница в один-два попадания при таких n лежит внутри случайности. Записано,
   чтобы никто не построил на этом вывод.
3. Список кандидатов вырос с 12 до 15, и читателям прямо сказано, что формат
   файла ответа не даёт (это правда: у OpenFake есть кадры veo-3, sora-2 и wan,
   а полосы сделаны тремя разными движками). Часть прежней точности опиралась
   на раскол видео/картинка, и эта опора убрана сознательно.
4. Civitai 0.1111 — почти случайность, и метка там от загрузчика. Отдельной
   строкой, в заголовок не идёт.
5. Содержание источников различается: у Civitai уклон в портрет и «арт», у Kling
   в сток-сцену. Снять без ручной курации нельзя; читатель может опереться.

---

## Почему промтов с Civitai всего 473 (вопрос владельца, ИЗМЕРЕНО)

Ответ оказался не тем, что ожидалось. Дело НЕ в API и НЕ в размере прогона —
хотя и они малы: `collect_civitai.py` по умолчанию `--pages 1 --versions 25`,
то есть 473 строки набраны несколькими крошечными прогонами.

Главное другое. `scripts/measure_civitai_funnel.py`, три страницы по 100 моделей,
Most Downloaded / AllTime:
    ...
    у отсеянных:  25 версий → 232 картинки → 138 годных       = 5.5 на версию

    на этих 300 моделях при нынешнем фильтре      ~870 промтов
    без МОДЕЛЬНОГО фильтра, только по картинке  ~22 300 промтов

**Модельный фильтр стоит примерно 96% доступного объёма.** И отсеянные модели
промтовее прошедших в 3.4 раза: это популярные общественные чекпойнты с
большими витринами.

ЧТО ИМЕННО ОН ДЕЛАЕТ. `ALLOWED_MODEL_LEVELS = 3` роняет версию, если МОДЕЛЬ
хоть где-то публикует материал выше PG-13 — даже когда сама собираемая картинка
имеет уровень 1 или 2. Это ВТОРОЙ, куда более грубый фильтр поверх поштучного
`MAX_NSFW_LEVEL = 2`, который и так работает по каждой картинке. У 138 из 205
отсеянных промтов картинка сама по себе потолок проходит.

Фильтр был поставлен обоснованно (запись в civitai.py: модель «NSFW MASTER» с
булевым `nsfw: False` и битмаской 31). Вопрос не в том, ошибка ли это, а в том,
нужен ли ДВОЙНОЙ фильтр. Это решение владельца, не техническое.

### archive.civitai — НЕПРОВЕРЕНО, хост закрыт шлюзом (Ц3)

    archive.civitai.com:443  → шлюз ответил 502 на CONNECT
    civitarchive.com:443     → шлюз ответил 403 на CONNECT

Обходить не стал. Отличить «домена не существует» от «домен закрыт политикой»
изнутри нельзя: локальный DNS резолвит только то, что разрешено (civitai.com
резолвится, эти два — нет). Если источник нужен, домены надо назвать владельцу
окружения; я их называю и на этом останавливаюсь.

Открытый путь, который ЕСТЬ: на huggingface.co лежат датасеты-выгрузки Civitai
(поиск `civitai` отдаёт список, HTTP 200). Их лицензии не проверял — по Ц5 это
делается ДО встраивания, а встраивать пока нечего.

---

## Обрезанный ответ винил формат. Дефект, убивший прогон

Первый большой сбор упал с «the model listing answered with something that is
not JSON». Правда — и не та причина.

ИЗМЕРЕНО 2026-08-31, `sort=Most Downloaded`, каждая модель тащит полный
HTML-`description`:

    limit=20    1 364 916 байт
    limit=50    3 502 039 байт
    limit=100   6 516 550 байт   ← а потолок в сборщике стоял 3 000 000

JSON ломался на символе 2 972 334. `fetch` читал ровно `max_bytes` и не мог
отличить тело, которое кончилось, от тела, которое обрезали.

ПОЧИНЕНО В ТРЁХ МЕСТАХ:
1. `fetch.read_capped(reader, max_bytes)` — читает на байт дальше потолка и
   возвращает `(тело, обрезано)`. Вынесено из `fetch` наружу (Т5): развилка
   внутри функции, открывающей сокет, тестом недостижима.
2. `fetch` кладёт в ответ `truncated` и `max_bytes`, и пишет об этом в note.
3. `civitai.collect` называет обрезание причиной, когда оно есть, и формат —
   когда обрезания не было. Негативный контроль обязателен: мусор, который
   не обрезан, обязан по-прежнему зваться мусором.

`LISTING_MAX_BYTES = 12_000_000` — ВЫБРАНО с запасом над 6 516 550 замеренными.

Заодно в `fetch` разведены две переменные `body`: тело успешного ответа (bytes)
и тело ошибки (str). Они делили одно имя, и mypy это поймал только сейчас.

Мутации (Т1), обе стороны, `__pycache__` вычищен:
    потолок листинга 12 МБ → 3 МБ            FAIL x1
    ветка «всегда обрезание» (if True)       FAIL x1
    ветка «всегда формат» (if False)         FAIL x1
    read(max_bytes) вместо read(max_bytes+1) FAIL x2
    порог len(body) > → >=                   FAIL x1
    восстановлено → 43 + 5 тестов, OK

### ПОПРАВКА к записи выше про «модельный фильтр стоит 96%»

Ошибка моя. `safe_models_only` по умолчанию **False** и всегда был — я мерил
фильтр, который не применяется. Корпус мал не из-за него.

ИЗМЕРЕНО, 300 моделей = 4443 версии, из них 3360 заявляют промты; уровни у 308
картинок с промтом: PG 57.1%, PG-13 19.8%, R 14.6%, X 6.2%, XXX 2.3%.

    потолок ≤2 (как было):  ~19 900 промтов уже с этих 300 моделей
    потолок ≤4 (решение):   ~23 700

То есть 473 строки — это следствие КРОШЕЧНЫХ ПРОГОНОВ (`--pages 1 --versions 25`
по умолчанию), а не фильтра. Решение владельца про R даёт +19%; сорокакратная
разница лежит в размере прогона.
