...
class ВыводШагов(unittest.TestCase):
    ...
    def test_порядок_канонический_а_не_по_словам_брифа(self) -> None:
        ...
        self.assertEqual(
            [op.name for op in pn.derive("сделай липсинк, а озвучку начитает диктор")],
            ["озвучка", "липсинк"],
        )

    def test_чужая_озвучка_не_заказывается_заново(self) -> None:
        """ПЕРЕПИСАНО 2026-09-03. Бриф «под мою озвучку» выводил ШАГ синтеза
        речи — под речь, которая у заказчика записана. Слово «озвучка» в нём
        называет ВХОД плана, а не работу; тот же список слов читается с двух
        сторон — `HAVE_AUDIO_CUES` и заслон `кроме` у операции."""
        бриф = "сделай липсинк под мою озвучку"
        self.assertEqual([op.name for op in pn.derive(бриф)], ["липсинк"])
        self.assertIn(pn.ARTEFACT_AUDIO, pn.inputs_of(бриф))
        # Негативный контроль: обычный заказ озвучки шагом остаётся.
        self.assertIn("озвучка", [op.name for op in pn.derive("нужна озвучка ролика")])

    def test_три_шага_из_настоящего_брифа(self) -> None:
        # Бриф владельца дословно. Озвучки в шагах нет намеренно: «он говорит
        # МОЮ озвучку» — звук у заказчика уже есть (см. тест выше).
        self.assertEqual(
            [
                op.name
...
class КлючиЗаказчика(unittest.TestCase):
    ...
    def test_их_ровно_столько_сколько_названо(self) -> None:
        # Литералы (Т2): четыре первых поля — запрет на вход, кадр,
        # длительность и цена. Все четыре отвечают на «можно ли вообще», а не
        # на «насколько хорошо». Длительность встала сюда 2026-09-03: ролик
        # длиннее предела не выйдет вовсе.
        self.assertEqual(pn.CONSTRAINT_KEYS, 4)
        self.assertEqual(
    ...
    def test_у_каждого_поля_ключа_есть_своё_значение_для_печати(self) -> None:
        """`_поле` держит СВОЙ список значений, и он обязан идти в ногу с
        `KEY_FIELDS`. 2026-09-03 длительность встала в ключ, а туда её не
        вписали — и строка «почему не сосед» назвала верное поле с ЧУЖИМ
        значением. Пока проверка сверяла только длину ключа, дефект молчал."""
        c = pn.Candidate("а", (), 0, 0, 0, "")
        напечатано = [pn._поле(c, i) for i in range(len(pn.KEY_FIELDS))]
        self.assertNotIn("?", напечатано)
        self.assertEqual(напечатано[pn.KEY_FIELDS.index("влезает ли по длительности")], c.dur_state)
...
class ПочемуНеСосед(unittest.TestCase):
    ...
    def test_называется_первое_разошедшееся_поле(self) -> None:
        # ОБА НА ОДНОЙ СТУПЕНИ ЗНАКА, И ЭТО УСЛОВИЕ, А НЕ ДЕТАЛЬ ФИКСТУРЫ:
        # иначе первым разошедшимся полем окажется ступень, и тест назовёт
        # чужое поле, оставаясь зелёным. `1` строка — это `SIGN_THIN`, `3` —
        # `SIGN_CLEAN`; после 2026-09-07 они разные, поэтому взяты 4 и 2.
        первый = pn.Candidate("а", (), 4, 1, 0, "", anchored=5)
        второй = pn.Candidate("б", (), 2, 1, 0, "", anchored=5)
        сказано = pn.why_not(первый, второй)
        self.assertIn("строк применимости", сказано)
        self.assertIn("4 против 2", сказано)
...
class ВытесненныйЦеной(unittest.TestCase):
    ...
    def test_без_потолка_вытеснить_некому(self) -> None:
        ...
        # Шаг берётся ПО ИМЕНИ, а не по номеру: с 2026-09-03 план дописывает
        # производителя нехватающего артефакта (здесь — озвучку под липсинк),
        # и нулевым шагом оказывается он.
        шаг = next(ш for ш in итог["steps"] if ш["step"] == "липсинк")
...
class ТриИсхода(unittest.TestCase):
    ...
    def test_разрыв_из_плана_больше_не_приходит_и_это_решение(self) -> None:
        """До 2026-09-03 бриф «сделай липсинк» давал класс `разрыв`: липсинку
        нужно видео, никто его не производит, заказчик о нём не сказал.
        Валидатор был прав, и продукт был бесполезен — ровно на том входе «с
        нуля», который назвал владелец.

        Теперь план ЗАМЫКАЕТСЯ: производитель дописывается и помечается. Из
        `plan()` класс `разрыв` не приходит вовсе, и это названо вслух, а не
        обнаружится потом. Сам класс сторожится по-прежнему — на пути
        `parse_pipeline`, где план пишет человек: `studio/mcp/tests/
        test_pipeline.py`, четыре случая. Артефакты `селфи` и `референс`
        объявлены `AMBIENT_ARTEFACTS`, `видео` и `аудио` закрывает замыкание,
        и других требований у словаря операций нет.
        """
        свои = list(ЗДОРОВЫЕ) + [
        ...
        # Требования словаря операций закрываются ровно двумя способами.
        закрыто = set(pl.AMBIENT_ARTEFACTS) | set(pn.ЗАКРЫВАЕТ_НЕХВАТКУ)
        нужно = {а for оп in pn.OPERATIONS for а in оп.requires}
        self.assertEqual(нужно - закрыто, set())

    def test_замыкание_дописывает_шаг_и_называет_это(self) -> None:
        """Дописанный шаг обязан быть ПОМЕЧЕН. Продукт, который молча
        дополняет заказ, врёт в ту же сторону, что и выдуманный факт."""
        свои = list(ЗДОРОВЫЕ) + [
            факт("тестовый-липсинк", "architecture", "lipsync talking-head", "vendor"),
            факт("тестовый-липсинк", "license", "apache-2.0", "vendor"),
        ]
        итог = pn.plan("сделай липсинк", facts=свои, today=СЕГОДНЯ)
        имена = [ш["step"] for ш in итог["steps"]]
        self.assertEqual(имена, ["озвучка", "генерация_видео", "липсинк"])
        дописанные = {ш["step"]: ш["inferred"] for ш in итог["steps"] if ш["inferred"]}
        self.assertEqual(sorted(дописанные), ["генерация_видео", "озвучка"])
        self.assertIn("аудио", дописанные["озвучка"])
        self.assertIn("видео", дописанные["генерация_видео"])
        # Заказанный шаг пометки не несёт: иначе пометка перестала бы
        # что-либо различать.
        липсинк = next(ш for ш in итог["steps"] if ш["step"] == "липсинк")
        self.assertEqual(липсинк["inferred"], "")

    def test_дописанный_шаг_виден_В_ПЕЧАТИ_а_не_только_в_поле(self) -> None:
        """Человек читает печать, а не поле ответа.

        ИЗМЕРЕНО 2026-09-03, П3: пометка `inferred` жила только в JSON, и
        печатный план показывал два шага, которых заказчик не называл, без
        единого слова об этом. Поле честное, печать — нет; чинилось то, что
        видит человек.
        """
        свои = list(ЗДОРОВЫЕ) + [
            факт("тестовый-липсинк", "architecture", "lipsync talking-head", "vendor"),
        ...
        текст = pn.render(pn.plan("сделай липсинк", facts=свои, today=СЕГОДНЯ))
        self.assertIn("шаг озвучка: ДОПИСАН ПЛАНОМ", текст)
        self.assertIn("шаг генерация_видео: ДОПИСАН ПЛАНОМ", текст)
        # Заказанный шаг такой строки НЕ получает.
        self.assertNotIn("шаг липсинк: ДОПИСАН ПЛАНОМ", текст)

    def test_якоря_шага_ловят_его_ремесло_а_не_чужое(self) -> None:
        """Якорь — константа-решение: им шаг набирает кандидатов.

        Т1, найдено мутацией 2026-09-03: подмена якоря `ambient-sound` на
        `video` у шага фоновых звуков НЕ КРАСНЕЛА НИЧЕМ — набор кандидатов
        этого шага не сторожил никто, и шаг молча начинал тянуть модели
        генерации видео. Тест держит границу на синтетической базе: рядом
        лежат модель фоновых звуков и модель видео, и второй в кандидатах
        быть не должно.
        """
        свои = [
            факт("тестовый-фоли", "architecture", "foley sfx ambient-sound", "vendor"),
            факт("тестовый-фоли", "license", "apache-2.0", "vendor"),
            факт("тестовый-видеогенератор", "architecture", "video text-to-video", "vendor"),
    ...
    def test_артефакт_на_входе_не_дописывает_шаг(self) -> None:
        """Граница дописывания со второй стороны: дописать лишнюю работу не
        дешевле, чем не дописать нужную."""
        свои = list(ЗДОРОВЫЕ) + [
            факт("тестовый-липсинк", "architecture", "lipsync talking-head", "vendor"),
            факт("тестовый-липсинк", "license", "apache-2.0", "vendor"),
        ]
        итог = pn.plan("сделай липсинк под мою озвучку", facts=свои, today=СЕГОДНЯ)
        дописанные = sorted(ш["step"] for ш in итог["steps"] if ш["inferred"])
        self.assertNotIn("озвучка", дописанные)
...
class СписокВходовИзСхемы(unittest.TestCase):
    """46 строк `accepts_inputs` лежали в базе с 2026-09-02 и не читал их
    никто: `allows_in` понимал только форму «accepts_… = yes», а канал схем
    пишет СПИСОК видов входа. Этим и объяснялась измеренная бесполезность
    связки «шагу нужен вход X → модель принимает X»: данные были, читателя
    не было.

    A/B на живой базе 2026-09-03: запретов 3 -> 15, явных разрешений 0 -> 4,
    «не сказано» 81 -> 65 из 84 мест кандидатов.
    """

    def _ф(self, значение: str, note: str = "") -> Fact:
        return Fact(
            model="м",
            attribute="accepts_inputs",
            value=значение,
            source_url="https://fal.ai/models/х",
            tier="portal",
            stated_on="2026-09-03",
            note=note,
        )

    def test_вид_есть_в_списке_вход_разрешён(self) -> None:
        состояние, нота = pn.input_list_stance([self._ф("аудио, видео")], ["видео", "аудио"])
        self.assertEqual(состояние, "вход разрешён явно")
        self.assertIn("accepts_inputs", нота)

    def test_вида_нет_в_полном_списке_это_запрет(self) -> None:
        """Схема эндпоинта перечисляет ВСЕ его поля: отсутствие поля — это не
        умолчание, а отказ принять такой вход."""
        состояние, нота = pn.input_list_stance([self._ф("видео, текст")], ["видео", "аудио"])
        self.assertEqual(состояние, "вход ЗАПРЕЩЁН")
        self.assertIn("аудио", нота)

    def test_неполный_список_не_запрещает(self) -> None:
        """Р1: канал схем сам помечает, что список собран не целиком, и такой
        список даёт «не сказано», а не «не принимает»."""
        состояние, _ = pn.input_list_stance(
            [self._ф("текст", note="полей-ссылок без опознанного вида 1 — список входов НЕПОЛОН")],
            ["видео"],
        )
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_артефакт_без_перевода_молчит(self) -> None:
        """`референс` бывает и картинкой и роликом, `бриф` принимает кто
        угодно: обещать по ним запрет значило бы вычёркивать годные модели за
        то, чего они не запрещали."""
        состояние, _ = pn.input_list_stance([self._ф("аудио")], ["референс"])
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_нет_строки_нет_суждения(self) -> None:
        состояние, _ = pn.input_list_stance([], ["видео"])
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_список_читается_по_ОБЪЯВЛЕННЫМ_входам_шага(self) -> None:
        """`step_inputs` дописывает `селфи` там, где вход несёт живое лицо —
        это про ЛИЦО В КАДРЕ, а не про поле-картинку у эндпоинта. Прочитав
        дополненный набор, канал запрещал `latentsync`, который принимает
        ровно то, что шагу нужно. Поймано фикстурами владельца до коммита."""
        факты = [self._ф("аудио, видео")]
        дополненный = ["видео", "аудио", "селфи"]
        объявленный = ["видео", "аудио"]
        по_дополненному, _ = pn.ban_stance(факты, дополненный)
        по_объявленному, _ = pn.ban_stance(факты, дополненный, объявленный)
        self.assertEqual(по_дополненному, "вход ЗАПРЕЩЁН")
        self.assertEqual(по_объявленному, "вход разрешён явно")

    def test_фразовый_запрет_старше_списка(self) -> None:
        """Строка «human faces cannot be uploaded» сужает список, а не спорит
        с ним: список говорит про ПОЛЯ, фраза — про содержимое."""
        факты = [
            self._ф("аудио, видео"),
            Fact(
                model="м",
                attribute="face_reference_restriction",
                value="real human faces cannot be uploaded as reference images or videos",
                source_url="https://вендор.test/x",
                tier="vendor",
                stated_on="2026-09-03",
            ),
        ]
        состояние, _ = pn.ban_stance(факты, ["видео", "аудио", "селфи"], ["видео", "аудио"])
        self.assertEqual(состояние, "вход ЗАПРЕЩЁН")


class ДлительностьВКлючеОтбора(unittest.TestCase):
    """Ось длительности решает ПОРЯДОК, а не только печать. ИЗМЕРЕНО
    2026-09-03: на ролике 30 с из 19 кандидатов с записанным пределом работу
    сделают два, а планировщик предлагал любого."""

    def _к(self, имя: str, состояние: str, применимость: int = 1) -> pn.Candidate:
        return pn.Candidate(имя, (), применимость, 0, 0, "", dur_state=состояние)

    def test_влезающий_старше_переросшего(self) -> None:
        влезает = self._к("влезает", "длительность принимает", применимость=1)
        длинный = self._к("длинный", "ролик ДЛИННЕЕ предела", применимость=9)
        # У переросшего применимость ВЫШЕ — и это не помогает: невозможное
        # старше любого нашего довода о качестве.
        порядок = sorted([длинный, влезает], key=pn.by_evidence)
        self.assertEqual([c.model for c in порядок], ["влезает", "длинный"])

    def test_незаписанный_предел_старше_переросшего(self) -> None:
        """«Не знаем» лучше, чем «точно не выйдет», и хуже, чем «выйдет»."""
        порядок = sorted(
            [
                self._к("переросший", "ролик ДЛИННЕЕ предела"),
                self._к("молчит", "предел длительности не записан"),
                self._к("влезает", "длительность принимает"),
            ],
            key=pn.by_evidence,
        )
        self.assertEqual([c.model for c in порядок], ["влезает", "молчит", "переросший"])

    def test_кадр_старше_длительности(self) -> None:
        """Обе оси — про невозможное, и порядок между ними назван вслух:
        кадр раньше, потому что модель, не принимающая кадр, не запустится
        вовсе, а переросший ролик можно порезать."""
        плохой_кадр = pn.Candidate(
            "кадр", (), 9, 0, 0, "", fit_state=pn.FIT_OVER, dur_state="длительность принимает"
        )
        плохая_длина = pn.Candidate(
            "длина", (), 0, 0, 0, "", fit_state=pn.FIT_IN, dur_state="ролик ДЛИННЕЕ предела"
        )
        порядок = sorted([плохой_кадр, плохая_длина], key=pn.by_evidence)
        self.assertEqual([c.model for c in порядок], ["длина", "кадр"])


class БезЧегоМодельНеЗапускается(unittest.TestCase):
    """«Принимает видео» и «БЕЗ видео не запустится» — разные утверждения, и
    второе решает судьбу шага. Завела эту ось прочитанная глазами выдача
    (П3, 2026-09-03): на шаге «генерация видео» стоял
    `kling-video-lipsync-text-to-video` с `required: [video_url, text,
    voice_id]` — модель требует ГОТОВОЕ видео, а шаг, который это видео и
    должен произвести, взять его неоткуда. По «принимает» она проходила.
    """

    def _ф(self, атрибут: str, значение: str) -> Fact:
        return Fact(
            model="м",
            attribute=атрибут,
            value=значение,
            source_url="https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=x",
            tier="portal",
            stated_on="2026-09-03",
        )

    def test_требует_того_чего_на_шаге_нет(self) -> None:
        состояние, нота = pn.required_stance([self._ф("requires_inputs", "видео, текст")], ["бриф"])
        self.assertEqual(состояние, "вход ЗАПРЕЩЁН")
        self.assertIn("видео", нота)

    def test_требует_ровно_то_что_шаг_даёт(self) -> None:
        состояние, _ = pn.required_stance(
            [self._ф("requires_inputs", "аудио, видео")], ["видео", "аудио"]
        )
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_текст_есть_всегда(self) -> None:
        """Задание заказчика приходит на каждый шаг: модель, которой нужен
        только текст, не запрещена нигде."""
        состояние, _ = pn.required_stance([self._ф("requires_inputs", "текст")], ["бриф"])
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_то_что_есть_у_заказчика_считается_доступным(self) -> None:
        """Готовый ролик закрывает требование `видео` даже там, где шаг его не
        объявляет: заказчик его уже принёс."""
        состояние, _ = pn.required_stance(
            [self._ф("requires_inputs", "видео")], ["бриф"], have=["видео"]
        )
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_строки_нет_нет_и_суждения(self) -> None:
        состояние, _ = pn.required_stance([self._ф("accepts_inputs", "видео")], ["бриф"])
        self.assertEqual(состояние, "о запрете на вход не сказано")

    def test_обязательное_старше_принимаемого(self) -> None:
        """Модель, которая принимает нужный вид И требует недоступный, —
        запрещена: невозможное старше пригодности."""
        факты = [
            self._ф("accepts_inputs", "видео, текст"),
            self._ф("requires_inputs", "видео, текст"),
        ]
