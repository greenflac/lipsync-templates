...
#: Байты, которые не декодируются в utf-8 и не разбираются в дерево. Это
#: НАСТОЯЩИЙ третий исход, а не выдуманный: так выглядит файл, сохранённый в
#: другой кодировке или недокачанный.
НЕЧИТАЕМЫЙ = b"\xff\xfe def (\n"


def _с_нечитаемым(содержимое: str | None = None, потолок: int = 0) -> dict:
    """Дерево, где рядом с нечитаемым файлом может лежать обычный."""
    with tempfile.TemporaryDirectory() as каталог:
        (Path(каталог) / "test_битый.py").write_bytes(НЕЧИТАЕМЫЙ)
...
class ТретийИсходНеСворачиваетсяНиВоЧто(unittest.TestCase):
    """Р1: «не смогли» — самостоятельный исход при ЛЮБЫХ соседях.

    НАЙДЕНО одиннадцатой приёмкой 2026-09-08: условие стояло
    `if не_смогли and not беды`, и один нечитаемый файл РЯДОМ с одной бедой
    под потолком давал печать «годно» при коде возврата 1 — зеркало того
    самого дефекта, ради которого этот гейт и заводился.
    """

    def test_нечитаемый_файл_это_не_смогли(self) -> None:
        итог = _с_нечитаемым()
        self.assertEqual(итог["unmeasured"], 1)
        self.assertEqual(итог["outcome"], "не смогли")

    def test_нечитаемый_рядом_с_бедой_под_потолком(self) -> None:
        """Беда под потолком — «годно», но нечитаемый сосед сильнее."""
        итог = _с_нечитаемым(КЛАСС_ПОСЛЕ_MAIN, потолок=1)
        self.assertEqual(итог["violations"], 1)
        self.assertEqual(итог["unmeasured"], 1)
        self.assertEqual(итог["outcome"], "не смогли")

    def test_нечитаемый_рядом_с_бедой_над_потолком(self) -> None:
        итог = _с_нечитаемым(КЛАСС_ПОСЛЕ_MAIN, потолок=0)
        self.assertEqual(итог["outcome"], "не смогли")

    def test_чистое_дерево_рядом_ничего_не_прячет(self) -> None:
        """Негативный контроль к трём выше: без битого файла — «годно»."""
        self.assertEqual(_на_дереве(ХВОСТ_ЧИСТ)["outcome"], "годно")


class СмотрелиНеВсёЧтоНашли(unittest.TestCase):
    """Р2: знаменатель — файлы, где main ЕСТЬ, а не все найденные.

    НАЙДЕНО той же приёмкой: печаталось «у 163 файлов из 197 main стоит
    последним», хотя у 6 файлов main нет вовсе — прибор уверенно говорил о
    том, чего не смотрел, и завышал собственное покрытие на 6 файлов.
    """

    def test_файл_без_main_не_попадает_в_знаменатель(self) -> None:
        with tempfile.TemporaryDirectory() as каталог:
            (Path(каталог) / "test_с_main.py").write_text(ХВОСТ_ЧИСТ, encoding="utf-8")
            (Path(каталог) / "test_без_main.py").write_text(
                "class Просто:\n    pass\n", encoding="utf-8"
            )
            итог = гейт.проверить(корни=(каталог,), потолок=0)
        self.assertEqual(итог["checked"], 2)
        self.assertEqual(итог["с_main"], 1)

    def test_на_живом_дереве_знаменатель_меньше_найденного(self) -> None:
        итог = гейт.проверить()
        self.assertLess(итог["с_main"], итог["checked"])
