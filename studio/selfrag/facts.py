...
#: ЗАПЯТАЯ ПЕРЕД ТРЕМЯ ЦИФРАМИ — РАЗДЕЛИТЕЛЬ ТЫСЯЧ, А НЕ ДЕСЯТИЧНАЯ ТОЧКА.
#: До 2026-09-05 `[.,]` читал `10,000` как «десять целых ноль тысячных», и
#: величины, различающиеся в ТЫСЯЧУ раз, объявлялись «одним ответом разной
#: подробности»: `'10'` против `'10,000'`, `'30'` против `'30,000 frames'`.
#: Нашла независимая проверка; на живой базе таких пар 0, то есть дефект пока
#: латентный — и сработал бы на первом же значении с тысячным разделителем.
#:
#: Разделитель тысяч ОТБРАСЫВАЕТСЯ вместе с группами: `1,500` — это 1500, и
#: сравнивать его надо с 1500, а не с 1.5.
_ВЕДУЩЕЕ_ЧИСЛО = re.compile(r"\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+(?:[.,]\d+)?")
...
def _ответ(value: str) -> str:
    ...
    if not найдено:
        return ""
    сырое = найдено.group(0)
    # Группы тысяч снимаются целиком; одиночная запятая — десятичная (европейская
    # запись `0,5`), и только она превращается в точку.
...
class Fact:
    ...
    #: КОГДА ЭТО БЫЛО СКАЗАНО ИСТОЧНИКОМ, если источник это сообщает сам.
    #: Пусто, когда узнать неоткуда.
    #:
    #: ЗАЧЕМ ОТДЕЛЬНОЕ ПОЛЕ. `stated_on` на 2560 строках из 4400 — ДЕНЬ НАШЕГО
    #: ЧТЕНИЯ. Для живой страницы вендора это правда: страница говорила это в
    #: тот день. Для СТАТЬИ — нет: препринт `2311.17982` вышел в ноябре 2023, а
    #: в базе стоит 2026-08-27, и утверждение трёхлетней давности выглядит
    #: свежим ровно как вчерашнее. ИЗМЕРЕНО 2026-09-05: из 223 строк с
    #: arxiv-идентификатором у 173 записанная дата ПОЗЖЕ месяца публикации,
    #: расхождение доходит до 2 лет 9 месяцев.
    #:
    #: И4: происхождение этого значения — РАСЧЁТ, а не ИЗМЕРЕНО. Первые четыре
    #: цифры arxiv-идентификатора — год и месяц подачи, это правило площадки, а
    #: не прочитанная нами дата. Поэтому день не выдумывается: значение имеет
    #: вид `ГГГГ-ММ`, и короче оно ровно потому, что точнее мы не знаем.
    #:
    #: Строки базы НЕ ПЕРЕПИСЫВАЮТСЯ: поле выводится при чтении, в одном месте
    #: (Е1). Переписать 173 строки значило бы подменить то, что записал сборщик,
    #: расчётом — и потерять различие между прочитанным и выведенным.
    published_on: str = ""

    #: ЗНАК строки применимости, объявленный ЯВНО: True — плохая новость, False
    #: — хорошая, None — не объявлен, и тогда знак выводится по имени атрибута
    #: (`factaxis.CONTRA_ATTRIBUTES`), как было до 2026-09-05.
    #:
    #: ЗАЧЕМ ПОЛЕ ЗАВЕДЕНО. Оплаченный замер sync-lipsync дал отрицательный
    #: результат, и он был записан под именем `holds_identity`, которого нет в
    #: списке отрицательных. Валидатор прочитал его как «применимость закрыта,
    #: не опровергнута» и объявил план ГОДНЫМ — первое «годно» в проекте
    #: оказалось ложным и получено из плохой новости. Знак, выводимый из имени,
    #: держится на аккуратности автора записи, а такое правило не держится.
    #:
    #: None ОСТАВЛЕН НАМЕРЕННО: 2100 строк базы поля не имеют, и переписывать
    #: их вслепую значило бы объявить знак там, где его никто не смотрел.
...
#: Год и месяц подачи в arxiv-идентификаторе: `2311.17982` -> ноябрь 2023. Это
#: правило площадки, действующее с 2007 года; для более старых идентификаторов
#: (`math/0309136`) образец не совпадает, и тогда вывода просто нет.
#:
#: ЗЕРКАЛА НАЗВАНЫ ПОИМЁННО, А НЕ ПОЙМАНЫ ПОДСТРОКОЙ. Тот же идентификатор
#: встречается в базе на huggingface (19 строк, ИЗМЕРЕНО 2026-09-05: 17 в
#: `papers-content/resolve/…`, 2 в `huggingface.co/papers/…`), и без них
#: половина статей осталась бы без даты источника. Искать «четыре цифры, точка,
#: пять цифр» где угодно в адресе нельзя: так под правило попадёт версия
#: библиотеки и номер сборки, и дата статьи будет выведена из чужого числа.
_ARXIV = re.compile(
    r"(?:arxiv\.org/(?:abs|pdf|html)|huggingface\.co/papers"
    r"|huggingface\.co/buckets/huggingchat/papers-content/resolve/\d{4})"
    r"/(\d{2})(\d{2})\.\d{4,5}",
    re.I,
)


def дата_публикации(source_url: str) -> str:
    """`ГГГГ-ММ` из arxiv-идентификатора, или пусто.

    День не выдумывается: идентификатор его не содержит, и дописать `-01`
    значило бы отдать расчёт за прочитанное.
    """
    совпало = _ARXIV.search(str(source_url or ""))
    if not совпало:
        return ""
    год = 2000 + int(совпало.group(1))
    месяц = int(совпало.group(2))
    if not 1 <= месяц <= 12:
        return ""
    return f"{год:04d}-{месяц:02d}"
        ...
        facts.append(
            (
                Fact(
                    ...
                    published_on=дата_публикации(str(row.get("source_url", ""))),
                    ...
                    # None, если ключа нет: «знак не объявлен» отличается от
                    # «объявлен хорошим», и сворачивать одно в другое здесь
                    # значило бы придумать знак 2100 старым строкам.
