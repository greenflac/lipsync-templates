...
СЕМЬИ: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "capabilities": {
        ...
        "exact": (
            ...
            # `watermark`, `moderation`, `consent_gate`, `face_requirement` и
            # `human_face_restriction` ПЕРЕЕХАЛИ ОТСЮДА 2026-09-07 в семью
            # `restrictions` — разбор в её комментарии. Коротко: они отвечали
            # на «что умеет» тем, чего модель НЕ делает.
            "grounding_search",
            "retrieval_grounding",
            "embedding_types",
            "endpoint_purpose",
    ...
    # ЧЕГО МОДЕЛЬ НЕ ДАЁТ. Заведена 2026-09-07 независимой приёмкой входа
    # «что умеет», и это единственный случай за два дня, когда заведённый
    # накануне синоним сделал ХУЖЕ, чем было до него.
    #
    # ИЗМЕРЕНО на живой базе (574 модели, `advise`, то есть путём
    # пользователя): «что умеет» отвечало у 27 моделей, из них с исходом
    # `pass` — 24, и у ЧЕТЫРЁХ из этих 24 (17%) ответ состоял ТОЛЬКО из
    # ограничений:
    #
    #   advise("minimax-h3", "что умеет")     -> pass
    #     moderation = «автоматическая модерация текста, картинок и видео»
    #   advise("sora-2", "что умеет")         -> pass
    #     human_face_restriction = «input images containing human faces are
    #     rejected; real people cannot be generated»
    #   advise("wan-2.7-t2v", "что умеет")    -> pass
    #     watermark = «console output carries an unremovable AI-generated mark»
    #   advise("elevenlabs-pvc", "что умеет") -> pass
    #     consent_gate = «can only be made of your OWN voice»
    #
    # То есть на вопрос «что умеет» продукт уверенно называл умением ровно то,
    # чего модель НЕ делает. До заведения синонима «что умеет» молчало, и
    # молчание было безопаснее: спросивший шёл искать дальше, а не уходил с
    # ответом наоборот.
    #
    # ПУТЬ ВЫБРАН ЗАМЕРОМ, А НЕ ВКУСОМ. Три пути прогнаны на одной базе:
    #
    #   ДО правки                       отвечает 27, только ограничениями 4
    #   убрать вход «что умеет»         отвечает  0, только ограничениями 0
    #   ранжировать внутри семьи        отвечает 27, только ограничениями 4
    #   ОТДЕЛЬНАЯ СЕМЬЯ (взято)         отвечает 23, только ограничениями 0
    #
    # Убрать вход — вернуть молчание всем 27, то есть заплатить 23 верными
    # ответами за 4 неверных. Ранжирование не чинит НИЧЕГО: когда весь ответ
    # состоит из ограничений, первым встаёт ограничение — порядок не меняет
    # состава. Разведение семей снимает все 4 и не теряет ни одной строки:
    # `check_reachability.py` печатает те же 0 недостижимых строк, потому что
    # каждое из пяти имён достаётся теперь своим вопросом.
    #
    # НЕГАТИВНЫЙ КОНТРОЛЬ СЕМЬИ (И5) — ЧТО СЮДА НАРОЧНО НЕ ВЗЯТО:
    #   `authenticated_read_reachable` = «no» — это УМЕНИЕ, чьё значение
    #       оказалось отрицательным; «не умеет» — честный ответ на «что умеет»,
    #       и уводить его отсюда значило бы прятать плохую новость (тот же
    #       разбор, что у `lipsync_identity_failure_mode` в семье поведения).
    #   `text_rendering_non_latin` = «undocumented» — это «не знаем», а не «не
    #       даёт»; третий исход принадлежит вопросу, на который отвечает (Р1).
    #   `license_restriction` и вся ценовая семья — ограничения ПРАВОВЫЕ и
    #       ДЕНЕЖНЫЕ, у них свои вопросы (`license`, `billing`) и свои решения
    #       по Ц5; смешать их сюда значило бы отвечать на «чего не даёт»
    #       лицензией и обратно.
    #   `input_restriction`, `face_reference_restriction` — про то, ЧТО
    #       ПОДАВАТЬ; они отвечают в семье входов, где плохая новость и есть
...
СИНОНИМЫ: dict[str, str] = {
    ...
    # === ЗАВЕДЕНО 2026-09-07 ПО ЗАМЕРУ ДОСТИЖИМОСТИ ЖИВЫМ ВОПРОСОМ =======
    #
    # ИЗМЕРЕНО. Гейт `scripts/check_reachability.py` печатал «не покрыто
    # семьёй 0 строк из 2127» — и это правда ровно про то, что он мерит: у
    # каждого имени атрибута ЕСТЬ семья. Но входом в тринадцать из тридцати
    # семей служило ТОЛЬКО имя самой семьи (`duration_floor`,
    # `measurement_method`, `model_identity`, `image_size`, `prompt_rule`,
    # `aspect_ratio`, `frame_rate`, `voice`, `editing`, `limits`, `billing`,
    # `capabilities`, `serving`) — наши внутренние слова, которых спрашивающий
    # не знает. На корпусе из 76 живых вопросов (ru+en) 51 не доставал НИЧЕГО,
    # а 148 строк базы из 2127 (7.0%) не доставал ни один из них:
    #
    ...
    #
    # То есть «покрыто семьёй» и «спрашивается продуктом» — РАЗНЫЕ величины, и
    # первая мерит вторую только при условии, что у семьи есть человеческий
    # вход. Ниже — этот вход для каждой семьи, у которой его не было.
    #
    # СЛОВО, КОТОРОЕ ЗВУЧИТ ДЛЯ ДВУХ СЕМЕЙ, СЮДА НЕ БЕРЁТСЯ (И5). Нарочно
    # отсутствуют: «качество» (это и `quality_options`, и `benchmark_score`, и
    # находки о протоколе), «размер» (и картинка, и вес файла, и число
    # параметров), «формат» (и пропорции, и контейнер выхода), «требования»
    # (и железо, и `requires_inputs`), «модель», «видео», «лучше». Синоним,
    # ведущий не в ту семью, хуже пропуска: пропуск молчит, а он отвечает.
    # деньги: СКОЛЬКО (семья `price`) и НА КАКИХ УСЛОВИЯХ (семья `billing`)
    "сколько стоит": "price",
    "оплата": "billing",
    "как платят": "billing",
    "условия оплаты": "billing",
    "тариф": "billing",
    "подписка": "billing",
    # как быстро считает МОДЕЛЬ и как быстро отвечает СЛУЖБА — разные семьи,
    # и это то же разделение, что записано у `generation_time` и `serving`.
    "как быстро": "generation_time",
    "регионы": "serving",
    "очередь": "serving",
    "где хостится": "serving",
    "queue": "serving",
    # длительность: потолок и пол — разные вопросы и разные семьи
    "сколько секунд": "max_seconds",
    "минимальная длительность": "duration_floor",
    "минимум секунд": "duration_floor",
    "min_duration": "duration_floor",
    "короче какого": "duration_floor",
    # кадр: пропорции, частота, размер картинки
    "пропорции": "aspect_ratio",
    "соотношение сторон": "aspect_ratio",
    "вертикальное": "aspect_ratio",
    "формат кадра": "aspect_ratio",
    "aspect": "aspect_ratio",
    "fps": "frame_rate",
    "частота кадров": "frame_rate",
    "сколько кадров": "frame_rate",
    "кадры": "frame_rate",
    "frames": "frame_rate",
    "размер картинки": "image_size",
    "размер изображения": "image_size",
    # голос. «озвучка» и «дикторы» — слова заказчика; «audio» СЮДА НЕ БЕРЁТСЯ:
    # оно стоит в `price_per_audio_second`, и это цена, а не умение голоса.
    "голос": "voice",
    "озвучка": "voice",
    "дикторы": "voice",
    "клонирование голоса": "voice",
    "tts": "voice",
    # правка готового
    "монтаж": "editing",
    "правка": "editing",
    ...
    "продлить": "editing",
    "продление": "editing",
    # как писать промт. «промт» ведёт СЮДА, а не в `text_limit`: сколько
    # символов влезает — другой вопрос, и у него своя семья.
    "промт": "prompt_rule",
    "промпт": "prompt_rule",
    "prompt": "prompt_rule",
    "как писать промт": "prompt_rule",
    # квоты и размеры запроса
    "лимиты": "limits",
    "квоты": "limits",
    "сколько за раз": "limits",
    "quota": "limits",
    # как её позвать
    "как позвать": "model_identity",
    "идентификатор": "model_identity",
    "версия": "model_identity",
    "endpoint": "model_identity",
    # что умеет и для чего
    "что умеет": "capabilities",
    "умеет": "capabilities",
    "возможности": "capabilities",
    "умения": "capabilities",
    # ЧЕГО НЕ ДАЁТ — отдельный вопрос и отдельная семья (заведено 2026-09-07,
    # разбор в комментарии семьи `restrictions`). «Что умеет» и «чего не даёт»
    # разведены ровно тем же решением, каким разведены «что принимает» и «что
    # отдаёт»: ответить на один другим значит солгать.
    "чего не даёт": "restrictions",
    "чего не даст": "restrictions",
    "ограничения": "restrictions",
    "что нельзя": "restrictions",
    "запреты": "restrictions",
    # «МОДЕРАЦИЯ», «ВОДЯНОЙ ЗНАК» И `watermark` СЮДА НЕ БЕРУТСЯ, И ЭТО ТОТ ЖЕ
    # НЕГАТИВНЫЙ КОНТРОЛЬ (И5). Каждое из них называет ОДНО имя семьи, а не
    # семью: спросивший «модерация» получил бы в ответ ещё и `watermark`, то
    # есть снова ответ не на свой вопрос — ровно тот дефект, ради которого
    # семья и заводится. Имя, спрошенное дословно, `expand` вернёт и без
    # синонима.
    "ниша": "positioning",
    "зачем": "positioning",
    "назначение": "positioning",
    # на чём построена и на чём крутится
    "на чём построена": "architecture",
    # «УСТРОЙСТВО» СНЯТО 2026-09-07 ТОЙ ЖЕ ПРИЁМКОЙ, ЧТО НАШЛА «что умеет».
    # По-русски это и СТРОЕНИЕ модели (`architecture`), и ЖЕЛЕЗО, на котором
    # она крутится (`runs_on`, `min_vram_gb`) — ровно та двусмысленность,
    # из-за которой сюда НЕ взято «требования» (см. список выше).
    # ИЗМЕРЕНО: «устройство» отвечало у 20 моделей, и у 7 из них рядом
    # записано железо, которое в ответ не попадало и о котором ответ молчал:
    #   minimax-h3     «устройство» -> architecture, а рядом runs_on
    #   hunyuan-video  «устройство» -> vae_compression, а рядом min_vram_gb
    # Слово выбирало одно из двух значений молча. Семья `architecture`
    # остаётся достижимой словами «на чём построена» и «архитектура».
    "видеопамять": "hardware",
    "gpu": "hardware",
    "какая карта": "hardware",
    "пойдёт ли на моей карте": "hardware",
    "language": "languages",
    "язык": "languages",
    # числа и доверие к числам — разные семьи (`benchmark_score` против
    # `measurement_method`): там ЧИСЛО, здесь — можно ли ему верить.
    "оценки": "benchmark_score",
    "score": "benchmark_score",
    "баллы": "benchmark_score",
    "чем мерили": "measurement_method",
    "как измеряли": "measurement_method",
    "методика": "measurement_method",
    "evaluation": "measurement_method",
    # жизненный цикл, популярность, лицо, вход-выход
    "устарела": "availability",
    "актуальна": "availability",
    "снята ли": "availability",
    "сколько скачиваний": "adoption",
    "насколько популярна": "adoption",
    "скачивания": "adoption",
    "держит ли лицо": "holds_identity",
    "похожесть": "holds_identity",
    "сходство": "holds_identity",
    "что на входе": "accepts_inputs",
    "что подавать": "accepts_inputs",
    "что на выходе": "produces_outputs",
    "что получится": "produces_outputs",
}
...
def норма(спрошено: str) -> str:
    """Одно написание вопроса: строчные, без краёв, пробелы = подчёркивания.

    ИЗМЕРЕНО 2026-09-07: синоним семьи `positioning` был записан как
    `для_чего`, и `advise("eleven_v3", "для чего")` отвечал «could not
    measure» при 33 записанных строках `positioning` в базе. Подчёркивание —
    наше написание, пробел — человеческое, и держать два ключа на одно слово
    значило бы завести второй источник истины (Е1): второй ключ разъедется с
    первым при первой же правке.

    СКЛЕЙКА ТОЛЬКО ЭТИХ ДВУХ РАЗДЕЛИТЕЛЕЙ, И ЭТО НЕГАТИВНЫЙ КОНТРОЛЬ (И5).
    Сравнение не становится подстрочным и не выбрасывает разделители совсем:
    «сколько стоит продление» обязано остаться БЕЗ семьи, а не собрать по
    кускам `price` и `editing` и ответить ценой на вопрос о монтаже.
    """
    return "_".join(str(спрошено or "").strip().lower().split())


def _синонимы_нормой() -> dict[str, str]:
    """`СИНОНИМЫ` в одном написании. Считается из них же, а не рядом с ними."""
    return {норма(к): з for к, з in СИНОНИМЫ.items()}
...
def семья(спрошено: str) -> str:
    ...
    """
    ключ = норма(спрошено)
    ключ = _синонимы_нормой().get(ключ, ключ)
