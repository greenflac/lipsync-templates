...
#: Не состояние задачи, а состояние ЖУРНАЛА: прочитать не смогли. Отдельным
#: словом, потому что «журнал молчит» и «журнал недоступен» — разные ответы, и
#: второй означает, что у оплаченной работы могла быть ручка, а мы её не видим.
НЕ_ПРОЧИТАН = "journal_unreadable"
...
#: Сколько байт хвоста журнала читается при поиске задачи. ВЫБРАНО: строка
#: занимает около 220 байт, задача — три строки, значит 8 МБ держат примерно
#: двенадцать тысяч последних задач. Больше — это уже разбор истории, и делать
#: его в обработчике запроса нельзя (см. `из_журнала`).
ОКНО_ЖУРНАЛА = 8 * 1024 * 1024
...
def записать_событие(
    ...
    """Дописать в журнал ручек событие, у которого нет своей задачи.

    ЗАЧЕМ ПУБЛИЧНАЯ ФУНКЦИЯ. Неудавшийся возврат кредитов случается ДО того,
    как задача заведена, или ПОСЛЕ того, как она settled, — записывать его
    некуда: в схеме сессии свободного поля нет, а заводить его значит менять
    чужой модуль ради одной строки. Журнал ручек уже переживает перезапуск и
    уже читается человеком, разбирающим деньги, — событие ложится туда же.
    """
    _записать(
        {
            "job_id": job_id,
...
def из_журнала(job_id: str, journal: Path | None = None) -> dict | None:
    ...
    путь = JOURNAL if journal is None else journal
    if not путь.exists():
        # Журнала нет вовсе — про эту задачу здесь ничего не записано.
        return None
    try:
        размер = путь.stat().st_size
        with путь.open("rb") as файл:
            # ЧИТАЕТСЯ ХВОСТ, А НЕ ВЕСЬ ФАЙЛ. Журнал дописывается три раза на
            # задачу и не имеет ни ротации, ни потолка; на 53 МБ полное чтение
            # стоило 0.8 с процессорного времени НА ЗАПРОС (ИЗМЕРЕНО
            # независимой проверкой 2026-09-05). Свежая задача лежит в конце,
            # и хвоста для неё достаточно.
            if размер > ОКНО_ЖУРНАЛА:
                файл.seek(размер - ОКНО_ЖУРНАЛА)
                файл.readline()  # выбросить обрезанную первую строку
            текст = файл.read().decode("utf-8", "replace")
    except OSError as беда:
        # ЖУРНАЛ НЕ ПРОЧИТАЛСЯ — ЭТО НЕ «ТАКОЙ ЗАДАЧИ НЕТ» (Р1). Раньше оба
        # случая возвращали `None`, и настоящая ОПЛАЧЕННАЯ задача после
        # перезапуска получала дословно ответ выдуманного идентификатора —
        # ровно то различие, ради которого журнал и заведён. Нашла независимая
        # проверка 2026-09-05, заняв путь журнала каталогом.
    ...
    for строка in текст.splitlines():
        ...
        if isinstance(row, dict) and row.get("job_id") == job_id:
            найдено = row
    if найдено is None and размер > ОКНО_ЖУРНАЛА:
        # НЕ НАШЛИ В ХВОСТЕ — ЭТО НЕ «НЕ БЫЛО» (Р1). Задача могла быть записана
        # раньше окна, и сказать «такой задачи нет» значило бы соврать про
        # оплаченную работу ради дешёвого ответа.
        return {
            "job_id": job_id,
            "state": НЕ_ПРОЧИТАН,
            "note": (
                f"искали в последних {ОКНО_ЖУРНАЛА} байт(ах) журнала из {размер}: "
                f"не нашли. Задача могла быть записана раньше окна"
            ),
        }
    return найдено
...
def status(job_id: str) -> dict:
    ...
    if record is None:
        ...
        if прежнее is not None and прежнее.get("state") == НЕ_ПРОЧИТАН:
            return {
                "job_id": job_id,
                "state": UNKNOWN,
                "outcome": UNMEASURED,
                ...
                "request_id": None,
                "note": (
                    f"про задачу {job_id!r} этот процесс ничего не знает, И ЖУРНАЛ "
                    f"РУЧЕК НЕ ПРОЧИТАЛСЯ ({прежнее.get('note')}). Это НЕ «такой "
                    f"задачи не было»: ручка к вендору могла быть записана и не "
                    f"видна нам. Работа для человека"
                ),
            }
