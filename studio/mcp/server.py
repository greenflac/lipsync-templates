...
def _нота_отказа(группы: list[str], есть_указание: bool, англ: bool) -> str:
    ...
    хвост = ". The plan is not built" if англ else ". План не строится"
    return ("; ".join(куски) + хвост) if куски else ""
...
def plan_pipeline(brief: str, creative: str = "", budget_usd: float = 0.0) -> str:
    ...
    if просев["outcome"] == UNMEASURED:
        ...
        # приёмкой 2026-09-07, и это регресс починки того же дня: ветка ниже
        # строила ноту только из `banned` и `injection`, а третий случай — «в
        # тексте нет ни одного слова» — не покрыт ни одной половиной. Заказчик
        # на пустом брифе получал «не годно [запрещённая_тема] —» с ПУСТЫМ
        # объяснением, совет про обращения к читателю, которых не было, и
        # «проверено 0, нарушений 0» рядом с отказом. Три правила разом: Р1
        # (третий исход свёрнут во второй), Р2 (ноль проверок при вердикте),
        # Е2 (причина названа не та, что случилась).
        return _json(
            {
                "outcome": UNMEASURED,
    ...
    if просев["outcome"] != PASS:
        # Строка о брифе печатается ВСЕГДА одна и та же и потому не смеет
        # утверждать о нём лишнего: причин не приводить его дословно две
        # (тема и обращение к читателю), а строка была написана про одну.
        ...
        return _json(
            {
                ...
                # ПОЛЕ НА КАЖДОМ ОТВЕТЕ — ВКЛЮЧАЯ ОТКАЗ. Восьмая приёмка:
                # собственное правило коммита было нарушено тем же коммитом —
                # `risks` подняли в безусловные только на пути успеха.
                "risks": просев["risks"],
                "risky": просев["risky"],
                # БРИФ НЕ ВОЗВРАЩАЕТСЯ ДОСЛОВНО. Проверка чужих указаний
                # существует затем, чтобы указание НЕ ДОЕХАЛО до читателя, а
                # читатель этого JSON — агент. Эхо полного текста на пути
                # отказа воспроизводило ровно то, ради чего проверка написана
                # (найдено приёмкой 2026-09-06). Заказчик свой бриф знает;
                # длина названа числом, чтобы ответ оставался сверяемым.
                "brief_words": просев["words"],
                "steps": [],
                # ПОЛЯ ТЕ ЖЕ, ЧТО У ВСЯКОГО ДРУГОГО ОТВЕТА ПЛАНА. НАЙДЕНО
                # продуктовой проверкой 2026-09-07: здесь не было ни `classes`,
                # ни `что_дальше`, и печать плана падала `KeyError` — то есть
                # единственный путь, где отказ надо ОБЪЯСНИТЬ, был единственным
                # непечатаемым. Ответ, у которого набор полей свой, читается
                # вызывающим через `.get` и молчит там, где обязан говорить.
                "classes": [],
                # ТЕМЫ НАЗЫВАЮТСЯ ГРУППАМИ, А НЕ СОВПАВШЕЙ ПОДСТРОКОЙ. Печать
                # признака давала «узнаваемые люди без их согласия: его лицо на
                # мо» — обрывок оборота из списка, который читается как
                # обрезанная цитата заказчика. Группа говорит то же и не
                # возвращает заказчику его же слова.
                # ЯЗЫК ОТКАЗА — ПО ЯЗЫКУ БРИФА. НАЙДЕНО восьмой приёмкой: это
                # единственная строка, по которой заказчик может действовать, и
                # английский заказчик её не читал — ровно тот дефект, ради
                # которого писалась вся работа над языком ответа.
                #
                # И СОВЕТ НАЗЫВАЕТ ОБЕ БЕДЫ, А НЕ ОДНУ: при отказе и по теме, и
                # по обращению к читателю прежняя редакция называла только
                # тему, заказчик чинил одно и получал отказ снова.
                "что_дальше": _что_дальше_по_отказу(
                    bool(просев["banned"]), bool(просев["injection"]), англ
                ),
                "question": None,
                "note": _нота_отказа(
                    _группы(просев["banned"], англ), bool(просев["injection"]), англ
                ),
            }
        )
