...
def _что_дальше_по_отказу(тема: bool, указание: bool, англ: bool) -> str:
    ...
    хвост = (" — and the plan will assemble" if англ else " — и план соберётся") + (
        (
...
def _общий_отказ(просев: dict, текст: str, хвост_ноты: tuple[str, str]) -> dict:
    ...
    return {
        ...
        "что_дальше": _что_дальше_по_отказу(
            bool(просев["banned"]), bool(просев["injection"]), англ
        ),
        "note": _нота_отказа(
        ...
        return _json(
            {
                **_общее_нечего_смотреть(
                    ...
                    brief,
                    "бриф_пуст",
                    "опишите работу словами — что нужно сделать и с чем; "
                    "по пустому брифу план не собирается",
                ),
                "classes": [],
                "steps": [],
                "question": None,
