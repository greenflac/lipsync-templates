...
def _группы(метки: list[str]) -> list[str]:
    """Русские имена ГРУПП без повторов, в порядке первой встречи."""
    имена: list[str] = []
    for м in метки:
        имя = screen.по_русски(str(м).partition(":")[0])
        if имя not in имена:
            имена.append(имя)
    return имена
        ...
        return _json(
            {
                ...
                "что_дальше": (
                    "перепишите заказ без этой темы — и план соберётся; "
                    "если тема в заказе главная, мы за неё не беремся"
                    if просев["banned"]
                    else (
                        "уберите из текста обращения к читателю ответа — "
                        "опишите работу, и план соберётся"
                    )
                ),
                "question": None,
                "note": (
                    (
                        "в словах заказчика названо то, чего студия не делает: "
                        + ", ".join(_группы(просев["banned"]))
                        + ". План не строится"
                        if просев["banned"]
                        else ""
                    )
                    + (
                        (". Кроме того, " if просев["banned"] else "")
                        + "в тексте есть обращение к читателю этого ответа: оно не "
                        "выполняется и дословно не приводится"
                        + ("" if просев["banned"] else "; план не строится")
                        if просев["injection"]
                        else ""
                    )
                ),
