...
class Потолок(unittest.TestCase):
    ...
    def test_потолок_ровно_двадцать_восемь(self) -> None:
...
class СписокНеПродуктаЖивойАНеДекоративный(unittest.TestCase):
    """Е1, найдено независимой проверкой 2026-09-06.

    `НЕ_ПРОДУКТ` не читался НИГДЕ: фильтр каталогов был написан вторым
    литералом внутри `модули()` и про `.claude` не знал. Мёртвая константа хуже
    отсутствующей — она читается как действующее правило, и следующий, кто
    впишет в неё каталог, будет уверен, что тот исключён.
    """

    def test_каталоги_названы_литералом(self) -> None:
        self.assertEqual(("tests", "fixtures", ".claude"), c.НЕ_ПРОДУКТ)

    def test_список_действительно_применяется(self) -> None:
        """Подмена `НЕ_ПРОДУКТ = ()` обязана менять поведение, иначе он
        декорация: каталог тестов вернулся бы в счёт долга."""
        деревня = Path(tempfile.mkdtemp())
        (деревня / "studio").mkdir()
        (деревня / "studio" / "tests").mkdir()
        (деревня / "studio" / "живой.py").write_text(
            "ПОРОГ = 5\n\n\ndef f(x):\n    return x > ПОРОГ\n", encoding="utf-8"
        )
        (деревня / "studio" / "tests" / "test_что_то.py").write_text(
            "ПОРОГ = 5\n\n\ndef f(x):\n    return x > ПОРОГ\n", encoding="utf-8"
        )
        with unittest.mock.patch.multiple(c, ROOT=деревня, _в_репозитории=lambda: None):
