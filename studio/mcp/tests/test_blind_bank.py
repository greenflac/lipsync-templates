...
class ЧтоСнимкуРазрешеноПронестиССобой(unittest.TestCase):
    """Список разрешённых ключей метаданных — это и есть слепота банка: всё, что
    в нём не названо, для валидатора носитель подсказки, чем бы оно ни звалось.
    Константа стояла без охраны, поймано ратчетом R7 2026-09-05.

    Ожидаемое — ЛИТЕРАЛЫ (Т2): набор написан руками. Импортированный из
    проверяемого модуля, он поехал бы вместе с ним и промолчал.
    """

    def test_разрешено_ровно_пять_ключей_разметки_jpeg(self):
        self.assertEqual(
            {"jfif", "jfif_version", "jfif_unit", "jfif_density", "dpi"},
            set(blind.ALLOWED),
        )

    def test_подсказка_в_метаданных_не_разрешена(self):
        """Негативный контроль (И5): прибор обязан ОТКАЗЫВАТЬ, а не только
        пропускать. Именно эти ключи несут ответ на вопрос, который задают
        читателю: чем сделано и каким промптом."""
        for ключ in ("prompt", "model", "software", "parameters", "comment", "exif"):
            self.assertNotIn(ключ, blind.ALLOWED, ключ)
