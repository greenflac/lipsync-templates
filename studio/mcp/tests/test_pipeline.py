...
class МутацииКонстант(unittest.TestCase):
    ...
    def test_маркеры_цены_слабее_топят_нижнюю_границу(self):
        ...
        # ЗНАЧЕНИЕ СМЕНЕНО 2026-09-05 С `0` НА `$0`, И ЭТО ТОЖЕ НЕ КОСМЕТИКА.
        # Разборщик цен перестал додумывать валюту: голое число, у которого ни
        # имя атрибута, ни текст не называют валюту, стало третьим исходом
        # вместо молчаливых долларов. Прежний `0` после этого перестал
        # разбираться в цену ВООБЩЕ — дыра закрылась с другой стороны, и вместе
        # с ней стал ненаблюдаем сторож `PRICE_MARKERS`: расширение списка
        # ничего бы не двигало, и мутация промолчала бы не потому, что список
        # сторожат, а потому, что фикстура больше не дыра. Доллар возвращает
        # наблюдаемость: строка про число кадров, которая ЯВЛЯЕТСЯ суммой в
        # долларах, но не является ценой, — ровно то, от чего список и стоит.
        факты = здоровые("модель-а", price="2.50") + [
    ...
    def test_каждая_запрещающая_формулировка_валит_шаг_в_одиночку(self):
        # ФИКСТУРА С ДВУМЯ ЗАПРЕТАМИ СРАЗУ — СЛАБЫЙ ТЕСТ. Здешняя `здоровые`
        # подаёт «research-only, non-commercial», и мутация «убрать
        # `non-commercial` из списка» ПРОМОЛЧАЛА 2026-09-04: строку продолжал
        # ловить сосед. На живой базе `non-commercial` — самая частая
        # формулировка запрета (ИЗМЕРЕНО: 17 строк из 278 о лицензии, против
        # 14 у `cc-by-nc` и 1 у `research only`), то есть молчал сторож
        # главного случая. Каждая формулировка проверяется ОДНА.
        for формулировка in pl.FORBIDDING_LICENCE_MARKERS:
            with self.subTest(формулировка):
                факты = здоровые("модель-а", licence=формулировка)
                отчёт = исход([шаг(requires=["селфи"])], факты)
                self.assertEqual(отчёт["classes"], ["лицензия"])
                self.assertEqual(отчёт["outcome"], "fail")

    def test_русское_имя_атрибута_лицензии_опознаётся(self):
        # ГРАНИЦА, НАЗВАННАЯ ВСЛУХ: русских имён атрибутов в живой базе 0 из
        # 284 (ИЗМЕРЕНО 2026-09-04), поэтому мутация «убрать кусок `лиценз`»
        # молчала — данных, на которых она видна, просто нет. Маркер оставлен
        # намеренно: имя атрибута пишет и наш собственный `record_model_fact`,
        # а не только чужой канал. Раз маркер живёт — он под тестом.
        факты = [
            факт("модель-а", "лицензия", "non-commercial"),
            факт("модель-а", "price_per_image_usd", "0.039"),
            *здоровые("модель-а")[2:],
        ]
        отчёт = исход([шаг(requires=["селфи"])], факты)
    ...
    def test_русское_имя_ценового_атрибута_опознаётся(self):
        # Та же граница с той же стороны: `цена`/`стоимост` в живой базе не
        # встречаются ни разу, и обе мутации молчали. Проверяется исходом, а
        # не разбором: цена выше бюджета обязана валить шаг классом «цена».
        #
        # ХВОСТ ИМЕНИ ОСТАВЛЕН АНГЛИЙСКИМ НАМЕРЕННО. Первая редакция теста
        # брала `цена_за_кадр_usd` и упала — но упала ПРАВИЛЬНО: строку
        # опознали ценовой, а сравнить с бюджетом не смогли, потому что «за
        # что» неизвестно, и валидатор честно сказал третий исход. Здесь
        # сторожится ОПОЗНАНИЕ имени, а не разбор единиц, поэтому «за что»
        # берётся в разбираемой форме.
        for имя in ("цена_per_image_usd", "стоимость_per_image_usd"):
            with self.subTest(имя):
                факты = [
                    факт("модель-а", "license", "apache-2.0"),
                    факт("модель-а", имя, "2.50"),
                    *здоровые("модель-а")[2:],
                ]
                отчёт = исход([шаг(requires=["селфи"], budget=0.30)], факты)
                self.assertEqual(отчёт["classes"], ["цена"])
...
class МутацииРазбораЦены(unittest.TestCase):
    ...
    def test_защита_снята_и_цена_за_миллион_снова_сравнивается(self):
        # ПЕРЕПИСАНО 2026-09-05. Прежняя редакция мутировала `pricing.PER`,
        # добавляя туда «million», и ждала, что цена за миллион токенов начнёт
        # сравниваться с бюджетом ОДНОГО ШАГА. То есть тест фиксировал как
        # норму ровно тот дефект, который в этот день и чинили: бюджет шага —
        # деньги за прогон, цена за токены — деньги за объём текста, и связать
        # их нельзя, не зная числа токенов.
        #
        # Теперь мутируется настоящая защита: `PER_NOT_PER_RUN`. Снимешь её —
        # ложное сравнение вернётся, и это обязано покраснеть.
        подача = ("модель-токены", "price_per_million_input_usd", ЦЕНА_ЗА_МИЛЛИОН_ТОКЕНОВ, 0.30)
        self.assertEqual(ценовой(*подача)["outcome"], "could not measure")
        with mock.patch.object(pl, "PER_NOT_PER_RUN", frozenset()):
