...
class АдресСтатьиОбязанБытьПроверяемым(unittest.TestCase):
    ...
    def test_признак_в_строке_запроса_чужого_адреса_не_считается(self):
        """Найдено независимой проверкой 2026-09-06: чужой блог нёс DOI в
        ЗАПРОСЕ и проходил как статья. Та же форма, что и `openreview.net`
        подстрокой, — чинится по форме (И7), а не по случаю."""
        итог = self.записать("https://blog.example.com/x?ref=10.1145/3592433")
        self.assertEqual(итог["outcome"], "fail")

    def test_идентификатор_arxiv_на_чужом_хосте_не_считается(self):
        """`vendor.example/pricing/2401.12345/` — не статья. Идентификатор
        засчитывается на площадке и на её зеркале, а не где угодно."""
        итог = self.записать("https://vendor.example/pricing/2401.12345/")
        self.assertEqual(итог["outcome"], "fail")

    def test_в_отказе_названо_то_что_правда_подошло_бы(self):
        """НАХОДКА 4 в её же коммите: `server.py` поправили, а текст отказа
        по-прежнему обещал, что PDF подойдёт. Пишущий читает отказ."""
        нота = self.записать("https://vendor.example/decks/marketing.pdf")["note"]
        self.assertIn("PDF признаком НЕ считается", нота)
        self.assertIn("DOI", нота)
