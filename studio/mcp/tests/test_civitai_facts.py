...
class Права(unittest.TestCase):
    def test_пустой_список_разрешений_помечается_некоммерческой(self) -> None:
        ...
        self.assertIn(
            "НЕКОММЕРЧЕСКАЯ: allowCommercialUse пуст — коммерческого права не дано", метки
        )
    ...
    def test_запрет_производных_считывается_отдельной_меткой(self) -> None:
        ...
        self.assertEqual(метки, ["производные запрещены: allowDerivatives false"])
...
class Заявки(unittest.TestCase):
    ...
    def test_значение_наблюдения_это_фраза_источника(self) -> None:
        строки = civitai.заявки(self._строка())
        наблюдение = [r for r in строки if r[1] == "observed_behaviour"][0]
        self.assertEqual(наблюдение[2], "Video generation works with as few as 6 steps.")
    ...
    def test_своё_рассуждение_уходит_в_ноту_а_не_в_значение(self) -> None:
        ...
        self.assertIn("описание автора модели на Civitai", наблюдение[4])
...
class Гейт(unittest.TestCase):
    def test_на_настоящих_фикстурах_исход_годно_и_проверок_не_ноль(self) -> None:
        ...
        self.assertEqual(итог["проверено"], 15)
