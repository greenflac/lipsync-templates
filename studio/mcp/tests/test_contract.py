...
class ЧужойЯзык(unittest.TestCase):
    ...
    def test_кириллица_это_не_смогли_а_не_нарушение(self) -> None:
        """`checked` стал 1, а не 0, и это ЧЕСТНЕЕ прежнего нуля.

        С 2026-09-06 на кириллическом промпте прибор всё же делает ОДНУ
        проверку — запрещённые темы просеиваются и по-русски. Три остальные
        (полоса слов, полоса фраз, запретная зона) по-прежнему не проверены, и
        `unmeasured` остаётся 3. Ноль здесь означал бы «не смотрели вообще», а
        это уже неправда.
        """
        ...
        self.assertEqual(out["banned"], [])

    def test_а_запрещённая_тема_по_русски_теперь_нарушение(self) -> None:
        """Вторая сторона того же (И5): «не смогли» на кириллице не должно
        покрывать то, что прибор всё-таки увидел."""
        out = contract.gate("обнажённая знаменитость, тёплый свет")
