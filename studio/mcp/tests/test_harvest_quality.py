...
class ГейтНаСвоейБазе(unittest.TestCase):
    ...
    def test_заход_по_телу_треда_опознан_и_не_гейтится(self) -> None:
        """Починенный канал попадает в СВОЮ группу, а не в «не смогли».

        Владелец правит сбор в этой же сессии: строки с меткой «тело треда #N»
        то появляются в базе, то откатываются, поэтому проверяется это на своей
        базе, а не на живой. Требования к группе нет намеренно — порог для
        чужого захода выбирает его автор, — но опознана она обязана быть: иначе
        метку можно поменять и целый канал молча уедет туда, где не проверяется
        ничего (Р2).
        """
        строки = [
            _строка(з, f"тред #{i}, состояние open", f"https://huggingface.co/м/discussions/{i}")
        ...
        строки += [
            _строка(
                "Can we get the diffusers structured format in place?",
                "тело треда #44, автор oglok, знак неясно",
                "https://huggingface.co/м/discussions/44",
            )
        ]
        строки += [_строка("話者が二人になる", "чужой", "https://example.com/страница")]
        with TemporaryDirectory() as каталог:
            ...
            группы = гейт.разложить(факты)
            вердикт = гейт.проверить(путь)
        self.assertEqual(len(группы[гейт.ПО_ТЕЛУ]), 1)
        self.assertEqual(вердикт["неопознанных"], 0)
        self.assertEqual(вердикт["outcome"], ГОДНО, вердикт.get("беды"))
