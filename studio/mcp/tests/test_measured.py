...
class ДвеПоловиныСводятсяБезМасок(unittest.TestCase):
    """Р1/Р2: неизмеримость ОДНОЙ половины не прячется за проверенностью другой.

    Найдено независимой проверкой 2026-09-06. Складывались `checked` обеих
    половин, и решение принималось по сумме. ИЗМЕРЕНО подменой: увести `STORE`
    на несуществующий файл — база измеренных чисел исчезает целиком, — и
    `--check` возвращал 0 со словами «проверять нечего». Прибор, сторожащий
    происхождение каждого числа проекта, молча переставал сторожить что-либо.
    """

    ЗАПИСИ_ЦЕЛЫ = {"outcome": PASS, "checked": 93, "violations": 0, "unmeasured": 0}
    ЗАПИСЕЙ_НЕТ = {"outcome": UNMEASURED, "checked": 0, "violations": 0, "unmeasured": 1}
    ХЭНДОФЫ_ЦЕЛЫ = {"outcome": PASS, "checked": 2, "violations": 0, "unmeasured": 0}
    ХЭНДОФ_РАЗДУЛСЯ = {"outcome": FAIL, "checked": 2, "violations": 1, "unmeasured": 0}

    def test_пропавшая_база_чисел_это_не_смогли(self):
        self.assertEqual(UNMEASURED, gate.свести_половины(self.ЗАПИСЕЙ_НЕТ, self.ХЭНДОФЫ_ЦЕЛЫ))

    def test_пропавшие_хэндофы_это_тоже_не_смогли(self):
        self.assertEqual(UNMEASURED, gate.свести_половины(self.ЗАПИСИ_ЦЕЛЫ, self.ЗАПИСЕЙ_НЕТ))

    def test_нарушение_сильнее_неизмеримости(self):
        """FAIL первым: найденное нарушение не прячется за «не смогли»."""
        self.assertEqual(FAIL, gate.свести_половины(self.ЗАПИСЕЙ_НЕТ, self.ХЭНДОФ_РАЗДУЛСЯ))

    def test_проверенных_ноль_это_не_годно_даже_при_ярлыке_годно(self):
        """Хвост свода: половина может назваться годной, ничего не проверив.

        Приёмка 2026-09-06: строка `PASS if checked else UNMEASURED` не
        охранялась ничем — подмена на голое `return PASS` держала тридцать
        тестов зелёными. От настоящих производителей она сегодня недостижима
        (обе половины при `checked = 0` возвращают UNMEASURED), то есть это
        МЁРТВАЯ страховка, а мёртвая страховка читается как живая. Теперь она
        достижима хотя бы по контракту функции — и потому охраняема."""
        пусто = {"outcome": PASS, "checked": 0, "violations": 0, "unmeasured": 0}
        self.assertEqual(UNMEASURED, gate.свести_половины(пусто, пусто))

    def test_обе_половины_целы_это_годно(self):
        """Негативный контроль (И5): свод, всегда отвечающий «не смогли», —
        не свод, а выключенный гейт."""
        self.assertEqual(PASS, gate.свести_половины(self.ЗАПИСИ_ЦЕЛЫ, self.ХЭНДОФЫ_ЦЕЛЫ))
