...
class ИсточникИзменился(unittest.TestCase):
    ...
    def test_отпечаток_разошёлся_это_работа(self):
        строки, сделано = refill.changed_work(
            self._журнал([self._запись("aaa"), self._запись("bbb")]), перечитаны={}
        )
        self.assertEqual((len(строки), сделано), (1, 0), строки)
    ...
    def test_отпечаток_прежний_работы_нет(self):
        self.assertEqual(
            refill.changed_work(
                self._журнал([self._запись("aaa"), self._запись("aaa")]), перечитаны={}
            ),
            ([], 0),
        )
    ...
    def test_одна_запись_это_не_изменение(self):
        ...
        self.assertEqual(
            refill.changed_work(self._журнал([self._запись("aaa")]), перечитаны={}), ([], 0)
        )
    ...
    def test_разные_способы_не_сравниваются(self):
        ...
        страниц разом."""
        строки, _ = refill.changed_work(
            self._журнал([self._запись("aaa", "сп-старый"), self._запись("bbb", "сп-новый")]),
            перечитаны={},
        )
        self.assertEqual(строки, [])

    def test_перечитанная_страница_из_очереди_уходит(self):
        """ИЗМЕРЕНО 2026-09-03: 11 страниц перечитаны, 56 утверждений сверены,
        очередь показывала те же 11. Очередь, где строки сделаны, учит себя не
        открывать."""
        строки, сделано = refill.changed_work(
            self._журнал([self._запись("aaa"), self._запись("bbb", дата="2026-09-02")]),
            перечитаны={"https://vendor.test/pricing": "2026-09-03"},
        )
        self.assertEqual((строки, сделано), ([], 1))

    def test_перечитанная_ДО_изменения_не_считается(self):
        """Читали прежнюю страницу: свидетельство об изменении моложе чтения."""
        строки, сделано = refill.changed_work(
            self._журнал([self._запись("aaa"), self._запись("bbb", дата="2026-09-02")]),
            перечитаны={"https://vendor.test/pricing": "2026-09-01"},
        )
        self.assertEqual((len(строки), сделано), (1, 0))
...
class СвереноСИсточникомРядомСВозрастом(unittest.TestCase):
    """Перечитывание ПОКАЗЫВАЕТСЯ, но порядка не двигает. Возраст факта
    считается от даты источника; засчитать перечитывание за свежесть значило
    бы решить продуктовый вопрос молча — а он задан владельцу и ответа нет."""

    def test_дата_сверки_дописывается_к_строке(self):
        строки = refill.с_состоянием_источника(
            [
                {
                    "reason": refill.STALE_VENDOR,
                    "model": "м",
                    "detail": "x, источнику 300 дней",
                    "where": "https://вендор.test/a",
                }
            ],
            {"https://вендор.test/a": ("источник не менялся", "2026-09-03")},
            перечитаны={"https://вендор.test/a": "2026-09-03"},
        )
        self.assertIn("сверено с источником 2026-09-03", строки[0]["detail"])
        self.assertIn("источнику 300 дней", строки[0]["detail"], "возраст остаётся на месте")

    def test_несверенная_строка_лишней_пометки_не_несёт(self):
        строки = refill.с_состоянием_источника(
            [
                {
                    "reason": refill.STALE_VENDOR,
                    "model": "м",
                    "detail": "x",
                    "where": "https://в.test/b",
                }
            ],
            {},
            перечитаны={},
        )
        self.assertNotIn("сверено", строки[0]["detail"])

    def test_порядок_от_сверки_не_меняется(self):
        """И5: пометка обязана быть видна и обязана НЕ решать. Две строки с
        одинаковой причиной стоят по имени модели, сверка тут ни при чём."""
        строки = refill.order(
            refill.с_состоянием_источника(
                [
                    {
                        "reason": refill.STALE_VENDOR,
                        "model": "b",
                        "where": "https://в.test/сверено",
                    },
                    {"reason": refill.STALE_VENDOR, "model": "a", "where": "https://в.test/нет"},
                ],
                {},
                перечитаны={"https://в.test/сверено": "2026-09-03"},
            )
        )
...
class ЧасыПротуханияПротивСверки(unittest.TestCase):
    """РЕШЕНИЕ 2026-09-03 (владелец сказал «сам решай»): дата факта остаётся
    датой ИСТОЧНИКА и сверкой не двигается, но ОЧЕРЕДЬ — список работы, и в
    ней несверенное идёт раньше сверенного."""

    def _строка(self, модель: str, сверено: str = "") -> dict:
        return {
            "reason": refill.STALE_VENDOR,
            "model": модель,
            "detail": "",
    ...
    def test_несверенное_идёт_раньше_сверенного(self):
        порядок = refill.order([self._строка("а-сверено", "2026-09-03"), self._строка("я-никогда")])
        self.assertEqual([с["model"] for с in порядок], ["я-никогда", "а-сверено"])

    def test_давняя_сверка_раньше_свежей(self):
        порядок = refill.order(
            [self._строка("а-свежая", "2026-09-03"), self._строка("я-давняя", "2026-06-01")]
        )
        self.assertEqual([с["model"] for с in порядок], ["я-давняя", "а-свежая"])

    def test_сверка_не_выбрасывает_строку(self):
        """Порог протухания не сдвигается: факт по-прежнему стар, и когда
        сверка сама состарится, строка поднимется обратно."""
        порядок = refill.order([self._строка("м", "2026-09-03")])
        self.assertEqual(len(порядок), 1)

    def test_причина_и_доедет_ли_старше_сверки(self):
        """Порядок ключей назван вслух: причина, потом «доедет ли до плана»,
        и только потом свежесть сверки."""
        порядок = refill.order(
            [
                {**self._строка("сверенный-кандидат", "2026-09-03"), "не кандидат": False},
                {**self._строка("несверенный-мимо"), "не кандидат": True},
            ]
        )
        self.assertEqual([с["model"] for с in порядок], ["сверенный-кандидат", "несверенный-мимо"])

    def test_дата_факта_сверкой_не_двигается(self):
        """Контроль на главное: очередь меняет ПОРЯДОК, а не возраст. Строка
        по-прежнему несёт свой возраст, каким его дал источник."""
        строки = refill.с_состоянием_источника(
            [
                {
                    "reason": refill.STALE_VENDOR,
                    "model": "м",
                    "detail": "x, источнику 300 дней",
                    "where": "https://в.test/a",
                }
            ],
            {},
            перечитаны={"https://в.test/a": "2026-09-03"},
        )
        self.assertIn("источнику 300 дней", строки[0]["detail"])
        self.assertEqual(строки[0]["сверено"], "2026-09-03")
