...
class УдалениеПерсональныхДанных(unittest.TestCase):
    """Единственная правка задним числом, которую сторож пропускает.

    2026-09-02 владелец решил убрать из базы ники практиков: 246 строк, 116
    разных ников, все в поле `note` в форме «автор <ник>». Дописыванием этого
    не сделать — строка уже написана.

    Пропуск сделан ПО СОДЕРЖАНИЮ, а не по номеру коммита, и вот на что это
    меняет: запись «в таком-то коммите можно» пропустила бы в том же коммите
    ЛЮБУЮ правку, включая подмену числа в чужом факте.
    """

    БЫЛО = '{"model": "m", "note": "тело треда #47, автор dzft3w, знак неясно"}'
    СТАЛО = '{"model": "m", "note": "тело треда #47, ник удалён 2026-09-02, знак неясно"}'

    def test_удаление_ника_пропускается(self):
        self.assertTrue(ao.только_ники([self.БЫЛО], [self.СТАЛО]))

    def test_правка_числа_рядом_с_удалением_ника_НЕ_пропускается(self):
        """Половина контроля, ради которой пропуск сделан по содержанию: в том
        же коммите нельзя протащить правку факта."""
        подмена = self.СТАЛО.replace("#47", "#48")
        self.assertFalse(ao.только_ники([self.БЫЛО], [подмена]))

    def test_обычная_правка_остаётся_нарушением(self):
        self.assertFalse(ao.только_ники(['{"a": 1}'], ['{"a": 2}']))

    def test_дописывание_это_не_удаление_ников(self):
        """Длина обязана совпасть: удаление ников не добавляет и не убирает
        строк, а дописывание разбирается своим правилом."""
        self.assertFalse(ao.только_ники([self.БЫЛО], [self.СТАЛО, '{"b": 1}']))

    def test_ничего_не_изменилось_это_не_удаление(self):
        """Иначе «ничего не поменялось» читалось бы как «мы вычистили данные»."""
        self.assertFalse(ao.только_ники([self.БЫЛО], [self.БЫЛО]))

    def test_сверка_с_деревом_пропускает_такую_правку(self):
        корень, _ = _репо([self.БЫЛО], [self.СТАЛО])
        с = ao.сверить("log.jsonl", rev="HEAD", repo=корень)
        self.assertEqual(с.outcome, PASS_)
        self.assertIn("персональных данных", с.почему)

    def test_сверка_с_деревом_НЕ_пропускает_соседнюю_правку(self):
        корень, _ = _репо([self.БЫЛО], [self.СТАЛО.replace("#47", "#48")])
        self.assertEqual(ao.сверить("log.jsonl", rev="HEAD", repo=корень).outcome, FAIL_)
