...
class ЦентыДелятсяНаСтоВОдномМесте(unittest.TestCase):
    ...
    def test_значащие_цифры_цены_не_теряются(self):
        """Подрезка ЗАЩИЩАЕТ от двоичного хвоста, но не смеет округлять саму
        цену. Обе прежние фикстуры имели ОДНУ значащую цифру, и подмена
        `.12g -> .3g` оставалась зелёной (Т1, измерено 2026-09-06): настоящая
        цена 0.0123456 цента молча стала бы 0.0000123, то есть ошибка в счёте
        заказчику. Фикстура взята с другого края (Т3) — цифр много."""
        запись, _ = опрос.deepinfra_record(
            {"model_name": "m", "pricing": {"cents_per_sec": 0.0123456}}, "2026-09-06"
        )
        self.assertEqual(запись["prices"][0]["amount"], 0.000123456)
