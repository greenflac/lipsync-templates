...
def gate(prompt: str) -> dict:
    ...
    # Запрещённые темы — через просев: он знает и кириллицу, и обороты, и
    # похожие буквы, а список студии берёт у студии (второй копии нет).
    from studio.mcp import screen  # локальный импорт: цикла нет

    banned = screen.просеять(text)["banned"]
    # ТЕКСТ ПРИВОДИТСЯ ТЕМ ЖЕ СПОСОБОМ, ЧТО И В ПРОСЕВЕ (Е1). Приёмка
    # 2026-09-06 обошла эту проверку двойным пробелом и дефисами: «ignore  all
    # previous» получал `pass` с нотой «instructions to the reader 0» — тогда
    # как та же строка в брифе отвергалась. Один кортеж, две реализации, разный
    # ответ на один вопрос: комментарий десятью строками выше называет это
    # дефектом, и он был здесь же.
    приведён = screen.привести(text)
    похоже = screen.привести_буквы(text)
