...
- `fork_e2e.similarity_source` → в отчёт стадии приёмки стиля. `shipped_similarity`
  ...
  ПОЗЖЕ: этого оказалось мало, и функция удалена — она была вторым оракулом
  об одном факте. Имя теперь возвращается вместе со значением, тем, кто его
  посчитал. См. запись «Instrument names» ниже.
