...
def решить(нужны: list[str], покрыты: list[str], поданы: list[dict[str, Any]]) -> str:
    """Исход по трём числам. Вынесено из `свести` (Т5) и чинит два дефекта.

    ПЕРВЫЙ (найден независимой приёмкой 2026-09-06 подменой метки): когда ни
    одна модель не осталась без применимости, `нужны` пуст, `покрыты` тоже — и
    прибор печатал `fail: моделей без применимости 0, из них с заявкой 0`.
    Ругань на пустом месте: сверять было нечего, а вердикт был «не годно».

    ВТОРОЙ: третий исход возвращал код 0 — то есть «не смогли» было
    неотличимо от «годно» для того, кто читает код возврата (Р2). Теперь
    неизмеримость даёт свой код.

    Ноль нужных замеров — это НЕ успех: успех здесь означает «на каждую модель
    без применимости подана заявка», а при нуле таких моделей утверждение
    пустое. Поэтому третий исход, а не первый.
    """
    if not нужны:
        return UNMEASURED
    if not поданы:
        return UNMEASURED
    return PASS if len(покрыты) == len(нужны) else FAIL
...
def свести() -> dict[str, Any]:
    ...
    исход = решить(нужны, покрыты, поданы)
    return {
        "outcome": исход,
        "checked": len(нужны),
