...
def run(path: Path = pn.DEFAULT_BRIEFS_PATH) -> dict[str, Any]:
    ...
    for c in случаи:
        for s in c["plan"]["steps"]:
            ...
            # ПОРОГ БЕРЁТСЯ У ПЛАНИРОВЩИКА, А НЕ ПЕРЕПИСЫВАЕТСЯ ЗДЕСЬ (Е1).
            # До 2026-09-07 тут стояло `applicability == 0` — своя копия
            # решения «что считать измеренным». Когда планировщик завёл
            # ступень `SIGN_THIN` («измерено ОДНОЙ строкой — чисто по
            # бедности»), проверка осталась при своём и объявила лишней
            # строку, которая как раз и была новостью: выбран кандидат с одной
            # строкой, а рядом стоит кандидат с тремя. Две копии одного
            # решения дали противоположные ответы на один вопрос.
            тонко = выбран["applicability"] < pn.ТОНКО
            if тонко:
                if not про_проверенных:
                    # Молчание законно ровно в одном случае: плотнее
                    # измеренного кандидата на шаге нет вовсе.
                    плотнее = [
                        a
                        for a in (s.get("alternatives") or [])
                        if int(a.get("applicability") or 0) >= pn.ТОНКО
                    ]
                    if выбран["applicability"] == 0 or плотнее:
    ...
    # ДЛИТЕЛЬНОСТЬ, числом по всем брифам разом. Ветка обязана быть ПРОЙДЕНА:
    # если ни на одном брифе ролик никого не перерос, ось не проверена вовсе,
    # и «выбран переросший 0» ничего не значит.
    переросли = 0
    шагов_с_длительностью = 0
    выбран_переросший: list[str] = []
    снятых = 0
    не_тот_выход = 0
    выход_подтверждён = 0
    ...
    for c in случаи:
        for s in c["plan"]["steps"]:
            ...
            длинных = int(s.get("too_long_count") or 0)
            переросли += длинных
            if длинных:
                шагов_с_длительностью += 1
            if выбран.get("dur_state") == "ролик ДЛИННЕЕ предела":
                выбран_переросший.append(f"{c['id']}/{s['step']} ({выбран['model']})")
            сколько = (
    ...
    return {
        ...
        "too_long_candidates": переросли,
        "steps_with_duration": шагов_с_длительностью,
        "chosen_too_long": выбран_переросший,
...
def verdict(итог: dict[str, Any]) -> tuple[int, list[str]]:
    ...
    for где in итог["chosen_too_long"]:
        беды.append(f"ВЫБРАН кандидат, который короче нужного ролика: {где}")
    if not итог["too_long_candidates"]:
        беды.append(
            "ни на одном брифе ролик не перерос ни одного кандидата: ось длительности "
            "не проверена вовсе (нужен бриф с названной длительностью и модель с пределом)"
        )
...
def render(итог: dict[str, Any]) -> str:
    строки = [
        ...
        (
            f"по длительности отодвинуто {итог['too_long_candidates']} кандидат(ов) "
            f"на {итог['steps_with_duration']} шаг(ах); выбран переросший "
            f"{len(итог['chosen_too_long'])}"
        ),
        (
