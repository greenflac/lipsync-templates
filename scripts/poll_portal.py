...
def полнота(путь: Path) -> tuple[bool, str]:
    """Лежит ли на диске ПОЛНАЯ очередь, и от какого числа. Нет файла — нет и полной."""
    if not путь.is_file():
        return (False, "")
    try:
        снято = json.loads(путь.read_text(encoding="utf-8"))
        ...
        return (False, "")
    return (not снято.get("partial"), str(снято.get("polled_on") or "без даты"))


def не_записывать(путь: Path, опрос: dict[str, Any]) -> bool:
    """Правда — неполный опрос НЕ должен затирать полную очередь на диске.

    ПОЙМАНО НА СЕБЕ 2026-09-02: портал ответил на 1 слово из 3, очередь
    записалась неполной (61 имя превратилось в 21), и сборка покраснела — гейт
    честно дал третий исход на неполной очереди. Гейт сработал верно, чинить
    надо причину: краснеть от чужой икоты не должна ни сборка, ни знание.

    Файл со знанием не заменяется на файл с меньшим знанием. Пометка `partial`
    и третий исход в `--check` остаются на месте — они сторожат случай, когда
    полной очереди на диске ещё нет вовсе и записать неполную всё же лучше,
    чем ничего, лишь бы она не притворялась полной.
    """
    полная, _ = полнота(путь)
    return полная and опрос["answered"] < опрос["asked"]
...
def main(argv: list[str]) -> int:
    ...
    if args.write and не_записывать(ОЧЕРЕДЬ, опрос):
        print(
            f"НЕ ЗАПИСАНО: опрос неполный (ответило {опрос['answered']} слов из "
            f"{опрос['asked']}), а на диске лежит полная очередь от "
            f"{полнота(ОЧЕРЕДЬ)[1]}. Знание не заменяется на меньшее знание; "
            "повтори опрос."
        )
    elif args.write:
        ОЧЕРЕДЬ.write_text(
