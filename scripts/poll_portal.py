...
def строка_разницы(опрос: dict[str, Any], новые: list[dict[str, Any]]) -> str:
    """Заголовок отчёта, НЕСУЩИЙ СВОЁ ОГРАНИЧЕНИЕ, а не перекладывающий его ниже.

    Развилка вынесена из точки входа (Т5): иначе её нельзя проверить, не
    печатая, и тест повторял бы её текст вторым способом (Е1).

    ИЗМЕРЕНО 2026-09-03, пять прогонов подряд: ответило 1, 2, 2, 3, 3 слов из
    трёх, и портал показал соответственно 22, 58, 58, 66, 66 моделей. Строка
    «база не знает 0» печаталась ОДИНАКОВО во всех пяти. Ноль на неполном
    обходе — это «мы не спросили», а не «на портале нет», и читатель, дошедший
    до первой цифры, разницы не видел.

    Оговорка ставится ТОЛЬКО на неполном обходе. Оговорка на полном — тот же
    шум, что её отсутствие на неполном: и то и другое учит не смотреть на
    строку.
    """
    заголовок = f"моделей на портале {len(опрос['found'])}, база не знает {len(новые)}"
    if опрос["answered"] >= опрос["asked"]:
        return заголовок
    return (
        f"{заголовок} — НО ОБХОД НЕПОЛНЫЙ "
        f"({опрос['answered']} слов из {опрос['asked']}): "
        "это счёт по спрошенному, а не по порталу"
    )
...
def main(argv: list[str]) -> int:
    ...
    print(строка_разницы(опрос, новые))
