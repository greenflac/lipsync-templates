...
ЧИСЛА НИЖЕ СНЯТЫ ПРИ ПОТОЛКЕ MAX_NSFW_LEVEL = 2 И ALLOWED_MODEL_LEVELS = 3.
Владелец поднял потолок до R (4) 2026-08-31, то есть ПОСЛЕ этого замера, и
пересчитывать его не нужно: он отвечал на вопрос «почему так мало» про то
состояние, в котором вопрос был задан. Скрипт при новом запуске возьмёт текущие
константы и напечатает текущие числа — расхождение с докстрокой ожидаемо и
означает, что потолок с тех пор двигали.
...
bl_ok, bl_all = yield_of(
    sample_b, "ОТСЕЯННЫЕ моделью (у самих картинок уровень в пределах потолка)"
)
print()
print(
