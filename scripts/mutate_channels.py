...
MUTANTS: list[tuple[str, str, str, str, str]] = [
    ...
    # === НЕ-ОТВЕТ В ПОЛЕ ЗНАЧЕНИЯ (studio/mcp/advice.py) =================
    # Мутируется сама развилка: если «не знаю» перестанет узнаваться, база
    # снова начнёт принимать незнание как факт — и оно снова будет отменять
    # измеренное (pass -> fail на настоящем факте).
    (
        "studio/mcp/advice.py",
        '    return приведено if приведено in НЕ_ОТВЕТЫ else ""',
        '    return ""',
        "база: незнание снова записывается как факт",
        "studio.mcp.tests.test_value_is_not_a_shrug",
    ),
    # Вторая сторона (И5): сравнение подстрокой отвергало бы настоящие
    # наблюдения («unknown artifacts at 8 s»), а ложный отказ здесь дороже
    # пропуска — отвергнутый факт не запишет никто.
    (
        "studio/mcp/advice.py",
        '    return приведено if приведено in НЕ_ОТВЕТЫ else ""',
        '    return next((н for н in НЕ_ОТВЕТЫ if н in приведено), "")',
    ...
    (
        ...
        "очередь: закрытые строки снова исчезают из отчёта, а не досчитываются",
    ...
    (
        "studio/planner.py",
        '            что = "; ".join(f"{ф.attribute} = {ф.value}" for ф, _ in прочие[:2])',
    ...
    (
        "studio/planner.py",
        "    в_валюте_бюджета = {в: с for в, с in за_ролик.items() if в == pl.BUDGET_UNIT}",
        "    в_валюте_бюджета = dict(за_ролик)",
        "цена: отказ снова выносится по чужой валюте (кредиты против долларов)",
        "studio.mcp.tests.test_acceptance_findings_4",
    ),
    (
        "studio/planner.py",
    ...
    (
        ...
        "    target = Path(text)",
        "креатив: путь с тильдой снова объявляется несуществующим",
        "studio.mcp.tests.test_acceptance_findings_4",
    ),
    # === ДОГАДКА ПРОТИВ ВОПРОСА И ЯЗЫК БРИФА ==============================
    (
        "studio/clarify.py",
        "            if артефакт_видео in доступно and артефакт_видео not in только_догадка:",
        "            if артефакт_видео in доступно:",
        "вопрос: догадка продукта снова закрывает вопрос о самой себе",
        "studio.mcp.tests.test_guess_is_not_an_answer",
    ),
    (
        "studio/clarify.py",
        "    только_догадка = set(дописано) - set(есть)",
        "    только_догадка = set(дописано) | set(есть)",
        "вопрос: спрашиваем даже там, где заказчик уже ответил",
        "studio.mcp.tests.test_guess_is_not_an_answer",
    ),
    (
        "studio/planner.py",
        '    "своё видео",\n    "свое видео",',
        '    "свое видео",',
        # СТОРОЖ ПЕРЕВЕДЁН 2026-09-07: мутант промолчал на 2700 тестах, потому
        # что ни один не подавал «своё видео» — притяжательные обороты не
        # покрываются клаузовым правилом (`свой`, `мой` — определения, не
        # слова владения). Четыре живые фразы заведены в набор.
        "вход: слова заказчика о готовом видео снова не читаются",
        "studio.tests.test_what_the_customer_brought",
    ),
    (
        "studio/planner.py",
        "                        if not _есть_кириллица(brief)",
        "                        if False",
        "ответ: англоязычному заказчику снова не сказано, почему не поняли",
        "studio.mcp.tests.test_guess_is_not_an_answer",
    ),
    # === ЦЕНА РОЛИКА И ОТРИЦАНИЕ В БРИФЕ (продуктовая проверка 2026-09-07) ==
    (
        "studio/planner.py",
        "        дороже = [в for в, с in в_валюте_бюджета.items() if с > потолок]\n        if дороже:\n            наибольшая",
        "        дороже = []\n        if дороже:\n            наибольшая",
        "цена: превышение потолка снова не замечается",
        "studio.mcp.tests.test_rate_is_not_a_price",
    ),
    (
        "studio/planner.py",
        'ЗА_СЕКУНДУ: frozenset[str] = frozenset({"second", "seconds", "sec", "секунда", "секунду"})',
        'ЗА_СЕКУНДУ: frozenset[str] = frozenset({"second", "seconds", "sec", "секунда", "секунду", "image"})',
        "цена: ставка за картинку объявлена ставкой за секунду",
        "studio.mcp.tests.test_rate_is_not_a_price",
    ),
    (
        "studio/mcp/screen.py",
    ...
    (
        ...
        "ПРОКЛАДКИ_ОТКЛЮЧЕНЫ: frozenset[str] = frozenset(",
        "бриф: окно отрицания в два слова открывает «без цензуры обнажёнка»",
        "studio.mcp.tests.test_negation_is_not_a_request",
    ),
    ...
    (
        ...
        "листы: вторая копия списка безобидных ключей вернулась — списки разъедутся молча",
        "studio.mcp.tests.test_casebank_strip",
    ),
    (
        "scripts/validator.py",
        ...
        'ALLOWED_INFO_KEYS = frozenset({"jfif", "jfif_version", "jfif_unit", "jfif_density", "dpi"})',
        "бенч: третья копия списка безобидных ключей вернулась",
        "studio.mcp.tests.test_casebank_strip",
    ),
    (
        "scripts/corpus_bundle.py",
        "        if Path(name).suffix.lower() in NEVER:",
        "        if False:",
        "связка: предохранитель не применяется — картинки и видео поедут в связку",
        "studio.mcp.tests.test_corpus_bundle",
    ),
    (
        "scripts/stop_named_not_asked.py",
        "CONTROL_OUTCOMES_MIN = 3",
        "CONTROL_OUTCOMES_MIN = 1",
        "хук: набору хватает одного исхода — третий исход перестаёт требоваться",
        "studio.tests.test_named_not_asked",
    ),
    (
        "studio/selfrag/learn.py",
        "TRUSTWORTHY_ROWS = 100",
        "TRUSTWORTHY_ROWS = 1",
        "выучка: тридцать прогонов объявляются основанием для перенастройки системы",
        "studio.selfrag.tests.test_facts_learn",
    ),
    (
        "studio/knowledge/ingest_gallery.py",
        "MIN_WORDS = 3",
        "MIN_WORDS = 1",
        "галерея: обломок разбора едет в базу как промпт",
        "studio.mcp.tests.test_ingest_gallery_rows",
    ),
    (
        "studio/knowledge/ingest_gallery.py",
        '    source_url = row.get("source_url") or row.get("url") or row.get("page")',
        '    source_url = row.get("source_url")',
        "галерея: адрес страницы теряется молча — точное удаление по просьбе владельца невозможно",
        "studio.mcp.tests.test_ingest_gallery_rows",
    ),
    (
        "scripts/find_signals.py",
        "MIN_MENTIONS = 3",
        "MIN_MENTIONS = 1",
        "подсказки: одно упоминание становится подсказкой — список из совпадений",
    ...
    (
        ...
        "        share_hit = count\n        share_miss = against",
        "подсказки: перекос считается счётчиками вместо долей — сторона побольше всегда выигрывает",
        "studio.mcp.tests.test_find_signals",
    ),
    (
    ...
    (
        "scripts/reprobe_hosts.py",
        'ОТКАЗ_ПОЛИТИКИ = re.compile(r"tunnel connection failed:\\s*(403|407)", re.I)',
        'ОТКАЗ_ПОЛИТИКИ = re.compile(r"tunnel connection failed", re.I)',
        "щуп: авария шлюза записывается отказом политики — закрываем себе хост чужой бедой",
        "studio.mcp.tests.test_reprobe_hosts",
    ),
    (
        "scripts/check_sheet_shape.py",
        "TOLERATED_UNIQUE = 0.25",
        "TOLERATED_UNIQUE = 0.9",
        "форма: допуск снят — лист выдаёт источник, и оценщику не надо смотреть на пиксели",
        "studio.mcp.tests.test_sheet_shape",
    ),
    (
        "scripts/measure_vae_text.py",
        "MIN_STROKE_PX = 5",
        "MIN_STROKE_PX = 1",
        "текст: мелкий кегль снова судится — возвращается ложная чистота",
        "studio.mcp.tests.test_measure_vae_text",
    ),
    (
        "scripts/build_casebank.py",
        "            if model not in CLOSED_MODELS or per_model[model] >= 2 or len(made) >= count:",
        "            if model not in CLOSED_MODELS or len(made) >= count:",
        "банк: с одной модели берётся сколько угодно — меряем вкус автора, а не модель",
    ...
    (
        ...
        "    темы = чужие",
        "просев: русские запрещённые темы снова не ищутся — гейт пуст на целевом языке",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/mcp/screen.py",
        "            места = [i for i, с in enumerate(по_порядку) if с == слово]",
        "            места = [i for i, с in enumerate(по_порядку) if с.startswith(слово)]",
        "просев: короткие слова сравниваются по началу — «труппа» становится насилием",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/mcp/server.py",
    ...
    (
        ...
        '    имя = словарь.get(группа.strip(), "")',
        "отказ: незнакомая группа проглатывается молча — заказчику не сказано ничего",
        "studio.tests.test_what_the_customer_brought",
    ),
    (
        "studio/mcp/screen.py",
        ...
        "отказ: имя группы уже её содержимого — заказчику говорят о том, чего он не писал",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/mcp/server.py",
        "            хвост=хвост_ноты[1] if англ else хвост_ноты[0],",
        "            хвост=хвост_ноты[0] if англ else хвост_ноты[1],",
        "отказ: языки хвоста переставлены — русский хвост в английском ответе",
        "studio.tests.test_answer_in_the_customers_language",
    ),
    # МУТАНТ НА `"violations": 0` УДАЛЁН 2026-09-07, а не покрыт тестом.
    # ИЗМЕРЕНО прогоном: ветка `_общее_нечего_смотреть` достижима только при
    # исходе просева «не смогли», где `просев["violations"]` и так ноль —
    # подмена литерала на выражение не меняет НИЧЕГО. Мутант, который нельзя
    # нарушить наблюдаемо, из таблицы удаляется (Ц7); сама строка остаётся
    # литералом нарочно: она говорит «здесь нарушений быть не может», а не
    # «здесь их посчитали».
    (
        "studio/mcp/server.py",
        '    пара = соберётся or (" — и план соберётся", " — and the plan will assemble")',
        '    пара = (" — и план соберётся", " — and the plan will assemble")',
        "совет: промпту снова обещают план — отвечают не о том, о чём спрашивали",
        "studio.tests.test_answer_in_the_customers_language",
    ),
    # ИМЯ ЖИВОГО ЧЕЛОВЕКА (2026-09-07). Обе стороны: не видеть никого и видеть
    # человека в любой паре заглавных.
    (
        "studio/mcp/screen.py",
        '        риски += [f"person\'s likeness: {имя}" for имя in _люди_в_заказе(строка)]',
        "        риски += []",
        "просев: «ролик, где Киану Ривз рассказывает про наш банк» снова без единого слова о правах",
    ...
    (
        ...
        "    if not any(\n",
        "    if False and any(\n",
        "просев: человеком объявляется любая пара заглавных — «Нижний Новгород» получает предупреждение",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        ...
        "        if False:",
        "просев: «реклама сети Narrative Coffee» объявляется заказом с живым человеком",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        ...
        "    свой = False",
        "просев: «наш актёр Иван Петров» получает предупреждение о чужих правах",
        "studio.tests.test_named_person_is_not_ours",
    ),
    ...
    (
        ...
        "    for группа, оборот in ():",
        "просев: описательный заказ дипфейка («его лицо на этом теле») снова проходит",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/mcp/screen.py",
        "    if not слова:",
        "    if False:",
        "просев: пустой текст снова объявляется чистым — «нарушений 0» при нуле проверок",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/mcp/contract.py",
        "        просев = screen.просеять(text)",
        '        просев = {"violations": 0, "banned": [], "injection": [], "note": ""}',
        "промпт: кириллица снова не судится — тот же текст получает два разных вердикта",
        "studio.mcp.tests.test_screen_customer_words",
    ),
    (
        "studio/duration.py",
    ...
    # ЛОЖНЫЙ ОТКАЗ ДОРОЖЕ ПРОПУСКА (2026-09-08, одиннадцатая проверка).
    # Три послабления и три негативных контроля к ним.
    (
        "studio/mcp/screen.py",
        "        and not _роль_без_имени(м, строка)",
        "        and not False",
        "просев: «Music video, singer lipsyncing to my track» снова отказ по теме",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
    ...
    (
        "studio/mcp/screen.py",
        "        and not _метка_про_аудиторию(м, строка)",
        "        and not False",
        "просев: английская половина заслона решает иначе, чем русская",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        "    if группа.strip().lower() != АУДИТОРИЯ_ГРУППА or not признак:",
        "    if not признак:",
        "просев: послабление про аудиторию расползлось на все группы сразу",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        "        аудитория = группа == АУДИТОРИЯ_ГРУППА",
        "        аудитория = True",
        "просев: «порно для детей» спасается правилом про аудиторию",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        '    без_пар = _ИМЯ_ФАМИЛИЯ.sub(" ", строка)',
        "    без_пар = строка",
        "просев: «промо для Talking Tom» — название объявляет человеком само себя",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
        '        без_терминов = без_терминов.replace(термин, " ")',
        "        без_терминов = без_терминов",
        "просев: «talking head ad for New York Pizza» — термин ремесла снова называет человека",
        "studio.tests.test_named_person_is_not_ours",
    ),
    (
        "studio/mcp/screen.py",
    ...
    # ВЫДАТЬ СДЕЛАННОЕ ЗА НАСТОЯЩУЮ ЗАПИСЬ (2026-09-08).
    (
        "studio/mcp/screen.py",
        '    найдено += [f"{ПОДДЕЛКА}: {о}" for о in ВЫДАТЬ_ЗА_НАСТОЯЩЕЕ if о in низ]',
        "    найдено += []",
        "просев: «fake video of a bank CEO» снова без единого слова о том, что это",
        "studio.mcp.tests.test_claim_is_the_customers_own",
    ),
    (
        "studio/mcp/screen.py",
        '    подделка = [р for р in риски if р.startswith(f"{ПОДДЕЛКА}: ")]',
        "    подделка = []",
