...
#: Пометка строки, чья модель НИ РАЗУ не попадает в кандидаты ни одной
#: операции студии. Работа по такой строке верна и никуда не доедет: план её
#: не увидит. ИЗМЕРЕНО 2026-09-03: из 13 показанных строк «протухший факт
#: прочих тиров» кандидатом не бывает НИ ОДНА модель — это Civitai-LoRA и
#: чужие эмбеддеры, попавшие в базу попутно.
НЕ_КАНДИДАТ = "модель не бывает кандидатом ни на одном шаге"


def модели_кандидаты(факты: list[Any] | None = None) -> set[str]:
    """Модели, которых планировщик показывает хоть на одном шаге.

    Считается ЖИВЫМ отбором, а не списком имён (Е1): второй способ узнать,
    кого продукт вообще рассматривает, разъехался бы с первым молча.
    """
    from studio import planner as pn
    from studio.factindex import FactIndex

    строки = факты if факты is not None else list(facts_mod.load_facts(FACTS))
    индекс = FactIndex(facts=строки)
    пустой = pn.Budget(None, "", (), "could not measure", "бюджет не спрашивали")
    имена: set[str] = set()
    for оп in pn.OPERATIONS:
        имена.update(к.model for к in pn.candidates_for(оп, индекс, None, пустой, None, None))
    return имена


def с_пометкой_кандидата(rows: list[dict[str, Any]], кандидаты: set[str]) -> list[dict[str, Any]]:
    """Дописать к строке, доедет ли её результат до плана.

    Пометка НЕ выбрасывает строку: «не кандидат сегодня» — не «не понадобится
    никогда», и решать за читателя, что ему не нужно, очередь не вправе. Но
    внутри одной причины такие строки уходят вниз: сперва то, что доедет.
    """
    готово: list[dict[str, Any]] = []
    for строка in rows:
        мимо = str(строка.get("model", "")) not in кандидаты
        готово.append(
            {
                **строка,
                "не кандидат": мимо,
                "detail": строка.get("detail", "") + (f" [{НЕ_КАНДИДАТ}]" if мимо else ""),
            }
        )
    return готово
...
def order(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Очередь: по приоритету причины, внутри причины — сперва то, что доедет
    до плана, затем по имени модели.
...
def main(argv: list[str]) -> int:
    ...
    кандидаты = модели_кандидаты()
    rows = order(
        с_пометкой_кандидата(
            с_состоянием_источника(stale, состояния) + asked + fresh + changed + portal,
            кандидаты,
        )
    )
