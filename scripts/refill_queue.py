...
def с_состоянием_источника(
    ...
    for строка in rows:
        ...
        готово.append(
            {
                **строка,
                # Дата сверки едет ОТДЕЛЬНЫМ полем, а не только в тексте:
                # по ней сортируется очередь (см. `order`), а разбирать
                # обратно собственную печатную строку — это второй способ
                # узнать известное (Е1).
                "сверено": сверено,
...
def order(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    до плана, затем давно не сверенное, затем по имени модели.

    ЧАСЫ ПРОТУХАНИЯ: РЕШЕНИЕ 2026-09-03, принятое мной по прямому «сам решай».

    Дата факта (`stated_on`) остаётся датой ИСТОЧНИКА и сверкой не двигается —
    это не обсуждается: страница, прочитанная сегодня и не менявшаяся с
    августа, августовская. Но ОЧЕРЕДЬ — список работы, а не список фактов, и
    в ней порядок обязан отражать, где работа ещё не сделана. Сверка с
    источником — сделанная работа.

    Поэтому внутри одной причины порядок такой:

        никогда не сверяли   ->  сверяли давно  ->  сверяли недавно

    Строка при этом НЕ исчезает и порог протухания не сдвигается: факт
    по-прежнему стар, и когда сверка сама состарится, строка поднимется
    обратно сама собой, без отдельного правила.
