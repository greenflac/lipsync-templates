...
def с_состоянием_источника(
    rows: list[dict[str, Any]], состояния: dict[str, tuple[str, str]]
) -> list[dict[str, Any]]:
    """Дописать к строкам очереди, что известно об их источнике."""
    готово: list[dict[str, Any]] = []
    for строка in rows:
        что, когда = состояния.get(str(строка.get("where") or ""), (ИСТОЧНИК_НЕ_НАБЛЮДАЛИ, ""))
        готово.append(
            {
                **строка,
                "detail": f"{строка.get('detail', '')} [{что}" + (f" {когда}]" if когда else "]"),
            }
        )
    return готово
