...
НАБОРЫ = (
    ...
    # Названное требование, про которое студия ничего не меряет (2026-09-08).
    "studio.tests.test_named_but_not_measured",
...
MUTANTS = [
    ...
    (
        "studio/planner.py",
        "    if источника and (not наша or наша[:7] > источника):",
        "    if False:",
        "даты: день нашего чтения снова выдаётся за дату статьи",
    ),
    (
        "studio/planner.py",
        '    if not части:\n        return ""',
    ...
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: между условием и возвратом встала ступень
        # `SIGN_THIN`. Мутируется само условие, смысл цели прежний.
        "    if not c.against:\n        # «Чисто» по одной строке",
        "    if c.against:\n        # «Чисто» по одной строке",
    ...
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: `готов` из списка убран (он дублировал
        # `ВЛАДЕНИЕ`), цель взята по первому оставшемуся обороту.
        '    "из ролика",\n    "мой ролик",',
        '    "нет-такого-оборота",\n    "мой ролик",',
        "HAVE_VIDEO_CUES -> строже: «из ролика» больше не вход плана",
    ),
    (
        "studio/planner.py",
        '    "из ролика",\n    "мой ролик",',
    ...
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: список `cues` у `звук_фон` развёрнут по
        # строкам и дополнен английскими оборотами, однострочная цель из
        # дерева исчезла. Цель взята по первой строке списка, чтобы
        # переживать дописывание в конец.
        '        name="звук_фон",\n        cues=(\n            "фоновые звук",',
        '        name="звук_фон",\n        cues=(\n            "нетакогослова",',
    ...
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: прежняя МУТИРОВАННАЯ форма стала живым
        # кодом после разделения условия — то есть мутант «сломал продукт»,
        # ничего не тронув. Ловится `check_mutants_clean`.
        '    if chosen.measured and not тонок:\n        return ""',
        '    if False:\n        return ""',
        "rival_line -> слабее: строка печатается и у проверенного выбранного (шум)",
    ),
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: условие разделено на два ради ступени
        # `SIGN_THIN`. Мутируется вторая половина — она и решает.
        '    if chosen.measured and not тонок:\n        return ""',
        '    if not chosen.measured and not тонок:\n        return ""',
    ...
    # СТУПЕНЬ «ЧИСТО ПО БЕДНОСТИ» (2026-09-07). Обе стороны порога и обе
    # стороны самой ступени: без неё одна строка зонда вытесняла четыре
    # измерения; с порогом «всё чисто — наверх» она вернулась бы.
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: ступень привязана к РОДУ строк, а не к числу.
        "        return SIGN_THIN if худое_измерение(c) else SIGN_CLEAN",
        "        return SIGN_CLEAN",
        "отбор: «чисто» по одной строке снова обгоняет четыре измерения",
    ),
    (
        "studio/planner.py",
        '    return not any(getattr(e, "kind", "") == fa.KIND_MEASUREMENT for e in строки)',
        "    return False",
        "отбор: зондовые строки снова считаются проверкой — починка живёт до следующего зонда",
    ),
    (
        "studio/planner.py",
        '    return not any(getattr(e, "kind", "") == fa.KIND_MEASUREMENT for e in строки)',
        "    return True",
        "отбор: тонким объявляется кто угодно — ступень перестаёт различать",
    ),
    (
        "studio/planner.py",
        '    строки = [e for e in c.evidence if getattr(e, "kind", "") in APPLICABILITY_KINDS]',
        "    строки = list(c.evidence)",
        "отбор: строками применимости считаются схемы и заявления вендора",
    ),
    (
        "studio/planner.py",
        "ТОНКО = 2",
        "ТОНКО = 1",
        "порог тонкости -> любая одна строка снова считается картиной",
    ),
    (
        "studio/planner.py",
        "ТОНКО = 2",
        "ТОНКО = 9",
        "порог тонкости -> тонким объявляется кто угодно, ступень перестаёт различать",
    ),
    (
        "studio/planner.py",
        "SIGN_THIN = 2\nSIGN_AGAINST = 3",
        "SIGN_THIN = 3\nSIGN_AGAINST = 2",
        "порядок ступеней -> тонкое чисто снова стоит выше целиком плохого",
    ),
    (
        "studio/planner.py",
        "    if chosen.measured and not тонок:",
        "    if chosen.measured:",
        "выдача: у тонко измеренного выбранного снова пропадает строка о плотном соседе",
    ),
    # ЧТО ЗАКАЗЧИК УЖЕ ПРИНЁС (2026-09-07). Обе стороны каждого заслона:
    ...
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: список развёрнут по строкам и дополнен
        # парами «глагол + to».
    ...
    (
        "studio/planner.py",
        "        if отсутствие >= 0 and владение < 0:",
        "        if False:",
        "вход плана: «исходников НЕТ» снова читается как «исходник есть»",
    ),
    (
        "studio/planner.py",
        "        if владение < 0:\n            continue",
        "        if False:\n            continue",
        "вход плана: любое упоминание видео объявляется входом — план молча укорачивается",
    ),
    (
        "studio/planner.py",
        "            гасящее = [о for о in _места(клауза, ОТСУТСТВИЕ) if о < место]",
        "            гасящее = []",
        "вход плана: «есть видео БЕЗ ЗВУКА» снова объявляет входом и аудио",
    ),
    (
        "studio/planner.py",
        '_КЛАУЗА = re.compile(r"[,;:.!?\\n]|\\s—\\s|\\s-\\s")',
        '_КЛАУЗА = re.compile(r"(?!x)x")',
        "вход плана: бриф не режется на клаузы — просьба из соседней фразы глушит владение",
    ),
    (
        "studio/planner.py",
        '        f"бриф: {итог[\'brief\']}"\n        if "brief" in итог',
        "        f\"бриф: {итог['brief']}\"\n        if True",
        "печать: отказ по запрещённой теме снова роняет render — заказчик видит трейсбек",
    ),
    # ЯЗЫК ОТВЕТА И ЧЕСТНОСТЬ СОВЕТА (2026-09-07, седьмая приёмка).
    # ДВА МУТАНТА ЭТОГО МЕСТА ПЕРЕЕХАЛИ ВНИЗ 2026-09-08. Развилка «относится ли
    # запись к вашему заказу» теперь решает не только оговорку, но и МЕСТО
    # цитаты в совете, и живёт в одной переменной `про_ваш_заказ` (Е1). Мутанты
    # на неё стоят в конце таблицы и красят обе стороны разом.
    (
        "studio/planner.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: строка встречается ДВАЖДЫ (набор записи и
        # набор брифа), подмена одной оставляла фильтр на второй, и мутант
        # молчал. Мутируется сам список — он и есть решение.
        '        "видео",\n        "ролик",\n        "секунд",',
        "",
        "относимость: общие слова считаются совпадением — оговорка исчезает у всех",
    ),
    (
        "studio/planner.py",
        "            больше = всего_кандидатов > сколько",
        "            больше = False",
    ...
    # ВТОРЫЕ СТОРОНЫ КОНСТАНТ (2026-09-07, по девятой приёмке): каждая из трёх
    # сторожилась только вниз, и мутант вверх молчал.
    (
        "studio/planner.py",
        "ЗАГЛАВНЫХ_СЛИШКОМ = 0.6",
        "ЗАГЛАВНЫХ_СЛИШКОМ = 0.95",
        "подсказки: почти-капс объявляется обычным регистром — заказ снова теряется",
    ),
    (
        "studio/planner.py",
        "    if len(слова) < 2:\n        return True",
        "    if len(слова) < 1:\n        return True",
        "подсказки: однословный латинский бриф судится долей, которой не из чего считать",
    ),
    (
        "studio/planner.py",
        "    return max(0, найдено - нельзя - 1)",
        "    return max(0, найдено - нельзя)",
        "совет: выбранный кандидат больше не вычитается — число завышено на один",
    ),
    (
        "studio/planner.py",
        "    return max(0, найдено - нельзя - 1)",
        "    return max(0, найдено - нельзя - 2)",
        "совет: вычитается лишний кандидат — число занижено на один",
    ),
    (
        "studio/planner.py",
        "ЗАГЛАВНЫХ_СЛИШКОМ = 0.6",
    ...
    # ЗАПРЕТ ОПЕРАЦИИ СЛОВАМИ (2026-09-07). Обе стороны и обе границы.
    (
        "studio/planner.py",
        "            _подсказка_есть(cue, низ, без_имён) and not _операция_отменена(низ, cue)",
        "            _подсказка_есть(cue, низ, без_имён)",
        "план: запрещённый заказчиком шаг снова в плане — «не генерируйте видео» игнорируется",
    ),
    (
        "studio/planner.py",
        "    return any(с in ОТМЕНА_ОПЕРАЦИИ for с in до) or any(о in после for о in ОТМЕНА_ПОСЛЕ)",
        "    return any(с in ОТМЕНА_ОПЕРАЦИИ for с in до + после.split())",
        "план: «не дороже 0.10» снова отменяет липсинк — отрицание цены принято за отмену операции",
    ),
    (
        "studio/planner.py",
        "ОКНО_ОТМЕНЫ = 2",
        "ОКНО_ОТМЕНЫ = 20",
        "план: окно отмены без края — любое «не» в брифе снимает операцию",
    ),
    (
        "studio/planner.py",
        "    while конец < len(низ) and низ[конец].isalpha():",
        "    while False:",
        "план: хвост берётся от конца ПОДСКАЗКИ — «озвучка не нужна» снова не читается",
    ),
    # СЕМЬ КОНСТАНТ, КОТОРЫЕ НЕ СТОРОЖИЛ НИКТО (2026-09-07, десятая приёмка).
    (
        "studio/planner.py",
        '    r"(?<![.!?:;]\\s)(?<!^)(?<!\\n)\\b[A-Z][A-Za-z0-9]*(?:\\s+[A-Z][A-Za-z0-9]*)*", re.M',
        '    r"(?<![.!?]\\s)(?<!^)(?<!\\n)\\b[A-Z][A-Za-z0-9]*(?:\\s+[A-Z][A-Za-z0-9]*)*", re.M',
        "подсказки: «Задача: Lipsync нашего видео» снова даёт совпало 0",
    ),
    (
        "studio/planner.py",
        "    первый = _ЦЕПОЧКА_ЗА_ЯРЛЫКОМ.sub(затереть, текст)",
        "    первый = текст",
        "подсказки: «Client: Talking Tom» снова заказывает липсинк",
    ),
    (
        "studio/planner.py",
        '        "studios",',
        "",
        "подсказки: «Talking Head Studios» снова заказывает липсинк",
    ),
    (
        "studio/planner.py",
        '        "inc",',
        '        "inc",\n        "video",',
        "подсказки: обычное слово объявлено формой компании — заказы съедаются",
    ),
    (
        "studio/planner.py",
        "    return sum(1 for c in found if c.fit_state == FIT_OVER)",
        "    return 0",
        "совет: отвергнутые кадром снова числятся доступными кандидатами",
    ),
    (
        ...
        '    "sync to ",',
        "",
        "вход плана: «lip sync to our audio» — принесённая дорожка снова не видна",
    ),
    (
        "studio/planner.py",
        "    по_английски = not _есть_кириллица(brief)",
        "    по_английски = False",
        "язык: английский заказчик снова получает вопрос и совет по-русски",
    ),
    (
        "studio/planner.py",
        "    по_английски = not _есть_кириллица(brief)",
        "    по_английски = True",
        "язык: русский заказчик получает ответ по-английски — прибор не различает язык",
    ),
    (
        "studio/planner.py",
        "    return max(0, найдено - нельзя - 1)",
        "    return max(0, найдено - 1)",
        "совет: число кандидатов снова завышено на тех, кем воспользоваться нельзя",
    ),
    (
        "studio/planner.py",
        "    return ШАГ_ПО_АНГЛИЙСКИ.get(имя, имя) if по_английски else имя",
        "    return имя",
        "совет: имя шага снова русское в английском ответе",
    ),
    (
        "studio/planner.py",
        '            принести = ЧТО_ПРИНЕСТИ.get(str(шаг.get("step") or ""))',
        '            принести = ("своё видео", "your own video")',
        "совет: «подать своё видео» снова обещается на шаге озвучки, где оно не поможет",
    ),
    ...
    (
        "studio/clarify.py",
        # ПЕРЕНАЦЕЛЕН 2026-09-07: между «стать» и «смонтиров» встал
        # `blog post` с комментарием, и прежняя цель из двух соседних строк
        # исчезла из дерева. Мутант, чья цель не находится, не мутирует
        # ничего и молча перестаёт сторожить — ловится `check_mutants_clean`.
        '    "смонтиров",\n    "монтаж",',
        "",
    ...
    # МУТАНТ ЗАМЕНЁН 2026-09-08 ПО ОТЧЁТУ ОДИННАДЦАТОЙ ПРИЁМКИ. Стоял «окно
    # назад 24 символа», и он ПРОМОЛЧАЛ на прогоне: код берёт `слова[-1]`, а
    # последнее слово перед подсказкой одно и то же при любом окне — окно
    # меняет результат, только если режет слово посередине. Мутант был
    # эквивалентным, то есть измерял ноль, а в отчёте коммита стоял как
    # заведённая охрана. Наблюдаемая развилка тут другая: смотреть СОСЕДНЕЕ
    # слово или ВЕСЬ текст перед подсказкой.
    (
        "studio/planner.py",
        "    return bool(слова) and any(слова[-1].startswith(с) for с in ВЫВЕСКА_ВПЕРЕДИ)",
        "    return any(с in низ[:место] for с in ВЫВЕСКА_ВПЕРЕДИ)",
        "строчная вывеска -> строже: родовое слово где угодно перед подсказкой съедает заказ",
    ),
    (
        "studio/planner.py",
        "    for слово in низ[конец:].split()[:ХВОСТ_НАЗВАНИЯ]:",
        "    for слово in низ[конец:].split()[:0]:",
        "строчная вывеска -> слабее: «avatar maker app» снова заказывает работу",
    ),
    (
        "studio/planner.py",
        "        if not чистое or чистое in СЛУЖЕБНЫЕ_ЗА_ПОДСКАЗКОЙ:",
        "        if not чистое:",
        "строчная вывеска -> строже: «voiceover for our app» читается вывеской и заказ теряется",
    ),
    (
        "studio/planner.py",
        "        if слово != чистое:\n            return False",
        "        if False:\n            return False",
        "строчная вывеска -> строже: поиск родового слова перескакивает через запятую",
    ),
    ...
    (
        ...
        "main последним -> слабее: третий исход сворачивается в «годно» при соседней беде",
    ),
    (
        "scripts/check_main_is_last.py",
        "        с_main += 1",
        "        с_main += 0",
        "main последним -> слабее: знаменатель снова все файлы, а не смотренные",
    ),
    # --- названо, но не мерится (заведено 2026-09-08) ---------------------
    (
        "studio/planner.py",
        '        совпало = next((п for п in признаки if п in низ), "")',
        '        совпало = ""',
        "названо-не-мерим -> слабее: требования заказчика снова не упомянуты нигде",
    ),
    (
        "studio/planner.py",
        '        совпало = next((п for п in признаки if п in низ), "")',
        "        совпало = признаки[0]",
        "названо-не-мерим -> строже: строка приписывается к любому брифу",
    ),
    (
        "studio/planner.py",
        '        "unmeasured_requirements": требования_без_ответа(brief, по_английски),',
        '        "unmeasured_requirements": [],',
        "названо-не-мерим -> слабее: счёт есть, а до заказчика не доезжает",
    ),
    (
        "studio/planner.py",
        '    названо = итог.get("unmeasured_requirements") or []',
        "    названо = []",
        "названо-не-мерим -> слабее: поле заполнено, а печать молчит",
    ),
    (
        "studio/planner.py",
        '        англ = not _есть_кириллица(str(итог.get("brief") or ""))',
        "        англ = False",
        "названо-не-мерим -> английскому заказчику строка по-русски",
    ),
    (
        "studio/planner.py",
        "            if ширина is not None and высота is not None\n"
        '            else "стороны неизвестны"',
        '            if True\n            else "стороны неизвестны"',
        "стороны кадра -> «креатив: NonexNone» снова в лицо заказчику",
    ),
    # --- место чужой записи в совете (заведено 2026-09-08) ----------------
    (
        "studio/planner.py",
        "            про_ваш_заказ = not свои or условия_записи_в_брифе(запись, свои)",
        "            про_ваш_заказ = True",
        "чужая запись -> телугу снова первой фразой на заказе про кофейню",
    ),
    (
        "studio/planner.py",
        "            про_ваш_заказ = not свои or условия_записи_в_брифе(запись, свои)",
        "            про_ваш_заказ = False",
        "чужая запись -> относящаяся запись уезжает вниз, читатель решает без неё",
    ),
    (
        "studio/planner.py",
        '                    f"{если_взять} We are saying this because the chosen model "',
        '                    f"{если_взять} " "" "",',
        "чужая запись -> на английском её просто выбрасывают, а не переставляют",
    ),
]
...
def _прогон() -> int:
    ...
    # КОД ВОЗВРАТА ЗАВИСИТ ОТ ИСХОДА (исправлено 2026-09-05 по независимому
    # аудиту). Здесь стоял безусловный `return 0`: прибор печатал
    # «ПРОМОЛЧАЛИ: …» и говорил гейту «пройдено». Это самый большой набор
    # проекта (113 мутантов), он гоняется ТОЛЬКО в CI, и покраснеть не мог
    # никогда — то есть сторожа константы-решения планировщика можно было снять
    # незаметно. У соседнего `mutate_channels.py` в тот же день стояло
    # `return 1 if молчали else 0`: одно правило, два исполнения, и жило то,
