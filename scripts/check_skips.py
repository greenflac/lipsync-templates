...
ABSENT_DATA = (
    ...
    "numpy not installed",
    # СНЯТО 2026-09-07. Здесь стояло "rewriter missing", и это была НЕ причина
    # «данных нет в окружении»: `studio/selfrag/rewriter.py` лежит в дереве
    # (36 866 байт), импортируется, и ИЗМЕРЕНО 2026-09-07 его пропуск не
    # срабатывает ни разу — `studio/selfrag/tests` даёт 347 прогнано, 0
    # пропущено. Легализована была ситуация «модуль исчез»: негативный
    # контроль (подменённый meta_path, ImportError на импорте) даёт 10
    # пропусков в `test_rewriter_contract.py`, и с этой строкой гейт считал
    # бы их честными. Отсутствие исходника — это выключённый тест, а не
    # отсутствующие данные (Т6); проверяется существованием файла, а не
    # словами.
