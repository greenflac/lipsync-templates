...
#: Пол на ЗНАМЕНАТЕЛЬ разницы: сколько имён база обязана знать, чтобы «нового
#: нет» вообще что-то значило. ИЗМЕРЕНО 2026-09-07 на закоммиченном
#: `model_facts.jsonl`: 549 свёрнутых имён. ВЫБРАНО 500 — запас вниз на
#: отзывы и слияния имён, но не на потерю базы.
#:
#: ЗАЧЕМ ИМЕННО ПОЛ. Разница считается вычитанием: `кандидаты - известные`.
#: Опустей известная сторона — и КАЖДАЯ модель индекса прочитается как новая;
#: канал напечатает сотню находок, ни разу не сказав, что сравнивать было не с
#: чем. Это ровно тот молча съехавший знаменатель, что дороже всего обходился
#: на этом проекте.
ПОЛ_ИМЁН = 500


def проверить_собранное(факты: Any = None, families: set[str] | None = None) -> dict[str, Any]:
    """Офлайн: годна ли ИЗВЕСТНАЯ СТОРОНА разницы. Сети здесь нет.

    ЧЕГО ЭТА ПРОВЕРКА НЕ ДЕЛАЕТ, И ЭТО НАПИСАНО ЗДЕСЬ, А НЕ ОБНАРУЖИТСЯ ПОТОМ.
    Канал ничего своего на диск не кладёт: он печатает разницу и забывает её.
    Значит офлайн проверяема ровно одна её сторона — та, что лежит в
    репозитории: имена базы и объявленные вендорские источники. Сетевая
    сторона (индексы HuggingFace и PyPI) здесь НЕ проверяется никак, и «годно»
    ниже про неё не говорит ничего.

    Нарушения (M):
      * свёртка имени базы не идемпотентна (`fold(fold(x)) != fold(x)`) —
        сравнение идёт по свёрнутому имени, и такая пара сравнивается сама с
        собой неверно;
      * два ОБЪЯВЛЕННЫХ семейства сворачиваются в одно имя — новая модель
        второго семейства спрячется за первым и в находки не попадёт.

    Не смогли (K): имён в базе меньше пола `ПОЛ_ИМЁН` — знаменатель разницы
    съехал, и любое «нового нет» перестало быть измерением.
    """
    имена = {fold(факт.model) for факт in (facts_mod.load_facts() if факты is None else факты)}
    семьи = known_families() if families is None else families
    нарушения: list[str] = []
    for имя in sorted(имена):
        if fold(имя) != имя:
            нарушения.append(f"свёртка имени не идемпотентна: {имя} -> {fold(имя)}")
    свёрнутые: dict[str, str] = {}
    for семья in sorted(семьи):
        ключ = fold(семья)
        if ключ in свёрнутые:
            нарушения.append(f"семейства {свёрнутые[ключ]} и {семья} сворачиваются в одно {ключ}")
        свёрнутые[ключ] = семья

    мало = len(имена) < ПОЛ_ИМЁН
    исход = FAIL if нарушения else (UNMEASURED if мало else PASS)
    заметка = (
        f"известная сторона разницы: имён базы {len(имена)} (пол {ПОЛ_ИМЁН}), "
        f"объявленных семейств {len(семьи)}. Сетевая сторона (индексы) здесь НЕ проверена"
    )
    if нарушения:
        заметка += "\n  " + "\n  ".join(нарушения[:10])
    if мало:
        заметка += (
            f"\n  ЗНАМЕНАТЕЛЬ СЪЕХАЛ: имён {len(имена)} против пола {ПОЛ_ИМЁН} — "
            "разница по такой базе назовёт новым то, что в ней уже было"
        )
    return {
        "outcome": исход,
        "checked": len(имена) + len(семьи),
        "violations": len(нарушения),
        "unmeasured": 1 if мало else 0,
        "note": заметка,
    }
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--check", action="store_true", help="без сети: проверить известную сторону разницы"
    )
