...
def frames_for_seconds(seconds: float, *, fps: int | None = None, bench: bool = False) -> dict:
    ...
    if not floor < seconds <= SECONDS_MAX if bench else not SECONDS_MIN <= seconds <= SECONDS_MAX:
        raise ValueError(
            ...
            + (" — стендовому описанию послаблена только нижняя граница" if bench else "")
        )
    ...
    return {
        ...
        "note": (
            f"{seconds} с при {fps} к/с = {requested} кадров, обёртка "
            ...
            + (
                ...
                else f". КОРОЧЕ ПОЛА {SECONDS_MIN} с — стендовый прогон, "
...
def window_plan(frames: int, *, window: int | None = None) -> dict:
    ...
    return {
        ...
        "window": window,
        "note": (
            f"{frames} кадров окнами по {window}: окон {windows}, "
