...
встраивания,). Повторять её вычисление здесь значило бы завести второй
способ узнать известное. Модуль принимает ГОТОВУЮ карточку, а чтение
картинки вынесено в точку внедрения `reader` — чтобы тесты не зависели ни от
внешнего пакета, ни от диска.
...
ТРИ ИСХОДА. `годно` — промт собран и лёг в измеренную полосу;
...
#:). Не «примерно два десятка слов», а снятое число.
WORDS_TARGET = 24
...
def from_image(path, *, reader=None) -> dict:
    ...
    if reader is None:
        def reader(p):
            ...
            # косметика: в проекте действует решение составителя шаблонов «`style.py` не
