...
Составитель пишет промт и подаёт демо-человека; на выходе — стилевой референс
с демо-личностью («эстетика»). Именно этот стиль дальше пробрасывается при
сборке референса клиента для видеомодели.

Два разных человека в двух разных шагах:

    составитель, один раз на шаблон:  промт + демо-личность -> эстетика
    клиент, каждый заказ:             фото клиента + эстетика -> рефка -> видео

Отсюда природа эстетики: это наш собственный кадр, и одежду с него при сборке
брать нужно — она и есть шаблон. А вот лицо с него брать нельзя никогда: это
единственная ось, где цена ошибки — чужой человек в ролике клиента.

Что здесь измеримо, а что нет. Измеряется одно: осталась ли на эстетике
демо-личность (ArcFace против демо-ассета). Если нет — промт перерисовал
человека, и эстетика не получилась, как бы красиво ни вышло. Попал ли кадр в
задуманный стиль — не измеримо ничем, что у нас есть: это судит глаз
составителя, и здесь так и написано.

Одно противоречие оставлено видимым, а не разрешено молча: промт `y2k`
называет «Adidas sneakers», `fisheye` — «Balenciaga trench», а запрет проекта
гласит «no brand names, no logos». Промты составителя не правятся — это его
материал; запрет добавляется отдельной строкой, конфликт виден в отчёте, а
решение остаётся за составителем — см. `brand_conflict`.
"""
...
#: База эстетик лежит ДАННЫМИ, а не кодом: промты — материал составителя шаблонов, и
#: правка промта не должна быть правкой модуля.
BASE_PATH = Path(__file__).resolve().parent.parent / "assets" / "fork_aesthetics.json"

#: ГЛАВНАЯ строка этого модуля. Промты составителя шаблонов описывают ЧУЖУЮ внешность
...
# ---------------------------------------------------------------------------
# АНТРОПОМЕТРИЯ. Решение составителя шаблонов: антропометрия вырезается вся
# ---------------------------------------------------------------------------
#
...
#: ВЫБРАНО (кем: этот модуль; из чего: обороты шести промтов составителя шаблонов).
#:
...
def strip_anthropometry(prompt: str) -> dict:
    ...
    # составителя шаблонов, и каждый модель читает как значащий: сдвоенный пробел и
...
# ---------------------------------------------------------------------------
# ЭСТЕТИКИ ПО ПОЛУ. Решение составителя шаблонов: эстетики делятся по полу
# ---------------------------------------------------------------------------
#
# ПОЧЕМУ. ИЗМЕРЕНО: гардероб переезжает с эстетики ВМЕСТЕ С ПОЛОМ.
...
# на живых клиентах. Составитель шаблонов уже ввёл ровно это правило для драйвингов вручную
...
#
# ПРАВИЛО МАШИННОЕ, А НЕ ЗАПИСКА: `pair_check` роняет несовпадение в «не
...
def gender_of(aesthetic) -> str:
    ...
    шаблона равен полу стилевой рефки.

    Читается ИЗ БАЗЫ, а не выводится из промта машиной: гардероб называет
    составитель шаблонов, и «мини-юбка» против «tailored bottoms» — его выбор, а не вывод
...
def brand_conflict(aesthetic: dict) -> dict:
    ...
    Раньше здесь был третий исход «решает составитель шаблонов»: промты
    называли «Adidas» и «Balenciaga», а запрет проекта запрещал названия
    марок. Составитель решил: бренды в промтах остаются, а во все промты
    стилей добавляется «no logo». Конфликта больше нет.
...
def compose(aesthetic, *, with_ban: bool = True, cut_body: bool = True,
    ...
    Порядок ВЫБРАН и не случаен: промт составителя шаблонов идёт ПЕРВЫМ и целиком, потому
    ...
    # описана в промте составителя шаблонов — у y2k это широкий угол вплотную, — и наша
...
def accept(*, made, demo, distances=None) -> dict:
    ...
    Лестница та же, что на всём проекте: 0.0652 тот же человек, планка
