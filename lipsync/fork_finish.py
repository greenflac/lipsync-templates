...
из которого вырезали окно. Оба решения составителя шаблонов от 2026-08-22 обязательны:
...
ЕДИНСТВЕННЫЙ ИСТОЧНИК ИСТИНЫ. Свой разбор ffprobe здесь НЕ ПИШЕТСЯ:
...
ТОЧКИ ВНЕДРЕНИЯ. Наружу модуль ходит только через ДВА параметра —
...
# ---------------------------------------------------------------------------
# КОНСТАНТЫ-РЕШЕНИЯ. У каждой помечено происхождение.
# ---------------------------------------------------------------------------

#: ВЫБРАНО (кем: составитель шаблонов, решение от 2026-08-22; из чего: из вертикальных
...
#: Коды возврата берутся у соседа, а не заводятся свои: 0 годно,
#: 1 не годно, 2 не смогли.
EXIT_BY_OUTCOME = fork_video.EXIT_BY_OUTCOME


# ---------------------------------------------------------------------------
# Чистые функции: развилки вынесены из точки входа, чтобы их красил тест.
# ---------------------------------------------------------------------------
...
def bias_from_columns(columns, *, ratio_w=TARGET_RATIO_W,
    ...
    литералах, не открывая ни одного видео.

    НЕГАТИВНЫЙ КОНТРОЛЬ ВСТРОЕН: на РОВНОЙ карте — пустой кадр, статика,
...
def audio_drift(expected_frames, actual_frames, *, fps) -> dict:
    ...
    ИСХОДА, а не назначается отдельно, поэтому «не годно с приклеенным
...
def finish(driving_path, kling_path, out_path, *, window, bias=0.0,
    ...
    # вердикт по тому, что ИСПОЛНИЛОСЬ. Файл опрашивается заново, и
