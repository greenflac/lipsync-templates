...
дешевле поймать до прогона, чем объяснить после.
...
research-репозитории, и заводить здесь второй раскодировщик значило бы завести второй
способ узнать известное.
...
Рядом с каждым вердиктом — три числа: `checked` сколько проверено,
...
РОНЯЮТ: составитель шаблонов посмотрел выход с 21% таких кадров и назвал кисти
правильными. Это ПОПРАВКА К ОЖИДАНИЮ по личности, а не критерий отказа, и
превращать её в отказ значит выбросить материал, который составитель шаблонов принял.

ТОЧКИ ВНЕДРЕНИЯ. Внешнего мира модуль касается ровно в шести местах, и
...
# ---------------------------------------------------------------------------
# Планки. НИ ОДНА не скопирована: чужое значение импортируется
# ---------------------------------------------------------------------------
...
#: переэкспортирует, а второе значение с тем же именем — это наизнанку.
from .identity_arcface import MIN_FACE_PX  # noqa: E402

#: КРИТЕРИЙ ПРИЁМА. ВЫБРАНО: решение составителя шаблонов (кем: составитель шаблонов; из
...
#: отказ). ВЫБРАНО 0.10 (кем: смена приёма; из чего: измерены ровно две
...
#:: она попадает и в вердикт, и в отчёт, и разъехаться им нельзя.
...
#: Коды возврата по исходу. Взяты у `fork_looper`, а не назначены заново:
...
# ---------------------------------------------------------------------------
# Точки внедрения: ЕДИНСТВЕННЫЕ места, где модуль трогает внешний мир
# ---------------------------------------------------------------------------
...
def read_faces(path) -> dict:
    ...
    было бы хуже (, и вторая модель в памяти), поэтому берётся тот же
...
def read_style_card(path) -> dict:
    ...
    косметика: в проекте действует решение составителя шаблонов «`style.py` не используется
...
# ---------------------------------------------------------------------------
# Чистые функции. Развилки вынесены из точек входа, чтобы их красил тест
# ---------------------------------------------------------------------------
...
def tally(checked: int, violations: int, unmeasured: int) -> dict:
    ...
    if checked == 0:
        ...
        # Это НЕ «годно»: непроверенное могло быть нарушением.
        out["outcome"] = UNMEASURED
    else:
        out["outcome"] = PASS
    return out
...
def parse_count_frames(text: str) -> dict:
    ...
    записанном ответе настоящего ffprobe, а не на живом вызове.
    """
    import json
    ...
    # `_ratio` берётся у `fork_video`: разбор `30000/1001` уже написан и
...
def is_orphan_wrist(points) -> bool | None:
    ...
    ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ, ЗАПИСАННЫЙ ЧИСЛОМ. Ожидание смены было «на
...
def face_size_verdict(sizes: list, no_face: int, unmeasured: int,
    ...
    # РЕШЕНИЕ СОСТАВИТЕЛЯ ШАБЛОНОВ: эта ось — ПРЕДУПРЕЖДЕНИЕ, а не отказ.
    #
...
def window_argv(video_path, out_path, start: int, end: int,
    ...
    тоже решение, и он обязан краснеть в тесте, а не только в прогоне.
...
def driving_intake(video_path, frame_paths=None, *, product_seconds=None,
    ...
    способом узнать известное. Без кадров оси 2-5 честно отвечают «не
...
def _report(kind, source, axes: dict, steps: dict, t0: float, *,
    ...
    ВЕРДИКТ СВОДА ВЫВОДИТСЯ ИЗ ОСЕЙ, а не назначается рядом с ними: при
