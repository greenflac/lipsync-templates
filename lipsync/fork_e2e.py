...
Ступени по одиночке проверены и измерены, а вместе до этого модуля не
собирались ни разу. Ручная сборка по мануалу дважды стоила денег: кусок
драйвинга уехал на два кадра длиннее заказанного, а 25-минутный прогон шёл
молча, упёрся в таймаут и унёс с собой всё, что успел измерить. Отсюда три
свойства, которые здесь важнее любой из ступеней:
...
Все внешние вызовы — точки внедрения: стилизация, загрузка файлов, вызов
Kling, соседние модули приёма и сборки приходят параметрами. Умолчания ходят
в сеть и стоят денег; тесты прогоняют весь путь на подставных функциях —
ни одного байта наружу.

Лестница, по которой читаются числа личности (все четыре точки измерены,
журнал замеров — в research-репозитории):
...
* pro-версии Kling исключены: $2.6880 против $0.2100 за тот же ролик
  (в 12.8 раза), а лейбл на одежде всё равно не выжил и фон получил лишнюю
  анимацию. Сторож `refuse_pro` роняет вызов до того, как потрачены деньги.
* У эндпоинта motion-control ровно три поля — проверено щупом с негативным
  контролем: знакомые поля с мусором отвергаются точной формулировкой,
  выдуманные — как несуществующие.
* Стиль задаётся стилизацией фотографии до Kling. Промтом в Kling стиль не
  работает: выходы с промтом и без совпали (similarity 0.9618/0.9744/0.9445
  при негативном контроле 1.0000).
* Бренды, логотипы и надписи на одежде не генерируем. Запрет входит в промт
  стилизации строкой `NO_BRANDS_CLAUSE`, и ступень 2 проверяет, что он там.

Чего этот модуль не делает: он не приём входов и не финальная сборка. Обе
ступени живут в соседних модулях (`fork_intake`, `fork_finish`) и зовутся
мягким импортом: нет модуля — исход `не смогли проверить` с именем модуля и
подсказкой, чем подменить, а не `не годно`. Отсутствие соседа — не брак
продукта.
"""
...
# Три исхода — ОДНИМ источником на весь проект. Копия строк здесь
...
# и второй способ узнать известное — дефект.
...
#: ИЗМЕРЕНО боевыми заказами (`work/bake_kling26_video.json`):
...
#: ВЫБРАНО (кем: составитель шаблонов; из чего: движение берётся с драйвинга, а личность с
#: фотографии — значит ориентируемся по видео).
CHARACTER_ORIENTATION = "video"

#: ИЗМЕРЕНО ДВАЖДЫ ПО БАЛАНСУ fal, и второе измерение исправляет первое.
#:
#:, трёхсекундные заказы: $0.2100 за вызов standard.
#:, волна 1 продуктовой длины: баланс 10.8490375 -> 10.1490375 за ДВА
...
#: ВЫБРАНО СОСТАВИТЕЛЕМ ШАБЛОНОВ: «надо подогнать под 10 долларов, сохранив 5 секунд».
PRODUCT_SECONDS = 5.0

#: Цена ПРОДУКТОВОГО ролика. Имя оставлено прежним намеренно: соседи читают
#: цену через него (`E.KLING_PRICE_USD`), и правка доезжает до них сама.
...
#: ИЗМЕРЕНО на восьми боевых заказах до включительно: 960x960, 30 к/с.
...
#: ВЫБРАНО (кем: поток E2E; из чего: `pro` исключён составителем шаблонов навсегда, и
#: запрет обязан быть машинным — строка в правилах не роняет заказ).
FORBIDDEN_TIERS = ("pro",)

#: ВЫБРАНО СОСТАВИТЕЛЕМ ШАБЛОНОВ ГЛАЗАМИ, ВОПРЕКИ ЧИСЛУ: `nanobanana-2` через
...
#:
#: ЧТО ПРОБОВАЛИ И НЕ СМОГЛИ. Встречную ось хотели снять `pose`:
...
#: ЧЕГО ЭТО ЧИСЛО НЕ ЗНАЕТ: оно снято на приборе составителя шаблонов замера. У другого
...
#: ВЫБРАНО (кем: составитель шаблонов; из чего: монтажный рез внутри одной сцены — это
#: дефект генерации, а не стиль). Планка самого прибора — `fork_looper.CUT_JUMP`
#: (4.0), и она берётся ОТТУДА, а не копируется сюда.
MAX_CUTS_OUT = 0

#: Запрет брендов. Решение составителя шаблонов, и оно в ПРОМТЕ, а не в голове: ступень 2
#: проверяет наличие этой строки и краснеет, если её вынули.
#: ПЕРЕСМОТРЕН решением составителя шаблонов: бренды в промтах ость остаются, просто
#: добавляем no logo во все промты стилей».
#:
#: ЧТО ИЗМЕНИЛОСЬ И ПОЧЕМУ. Прежняя редакция начиналась с «no brand names» и
#: этим ВОЕВАЛА С МАТЕРИАЛОМ СОСТАВИТЕЛЯ ШАБЛОНОВ: его собственные промты называют
...
#: составителя шаблонов, и в приёмке она записана именно так.
...
#: «стиль». Причина ИЗМЕРЕНА боевым прогоном: стилизация надела на
...
#: (`gpt-image-2`, забракован составителем шаблонов глазами).
...
#: ЛЕСТНИЦА ArcFace. ИЗМЕРЕНО на нашем материале и живёт ОДНИМ местом:
...
#: РЕШЕНИЕ СОСТАВИТЕЛЯ ШАБЛОНОВ, ПОСЛЕ боевого прогона: «очки сели — это не баг, а
...
#: запрещены составителем шаблонов как продуктовая позиция, а аксессуары — как ответ на
...
#: Ступени по порядку. Список — это и есть порядок прогона: печать,
...
# ---------------------------------------------------------------------------
# Прибор стиля. Свой, дешёвый, БЕЗ СЕТИ — и с обоими контролями
# ---------------------------------------------------------------------------
...
def shipped_similarity(left, right) -> float | None:
    ...
    ПОЧЕМУ ОН ЗДЕСЬ ОДИН. в дереве оказалось ДВА способа померить одно
    ...
    защищает. Пакет теперь виден среде, поэтому ОТГРУЖАЕТСЯ ВНЕШНИЙ, а
    этот остаётся запасным на случай, когда пакета нет: без него ступень
    ответила бы «не смогли» и стенд встал бы, как встал.
...
def palette_similarity(left, right) -> float | None:
    ...
    ИЗМЕРЕНЫ на нашем же материале:
    ...
    Оба контроля на месте: прибор умеет сказать «нет» (0.3170 на
...
def live_upload(path) -> str:
    ...
    `fal_client.upload_file` — путь, которым уехали боевые входы
...
def live_kling(*, video_url: str, image_url: str, character_orientation: str,
    ...
    исполнялся ни разу. Форма запроса и разбор ответа списаны с боевого
    прогона (`work/bake_order.py`, `work/bake_kling26_video.json`), где
...
#: `fork_intake`, а не угадано: у него нет одной общей точки входа,
...
def stage_intake(*, client_photo, style_ref, driving, intake=None,
    ...
    if intake is None:
        ...
        if all(callable(f) for f in trio):
            ...
            # и ступень честно встаёт на «не смогли». ИЗМЕРЕНО живым
...
def stage_stylize(*, client_photo, style_ref, out_path, stylize=None,
    ...
    (решение составителя шаблонов): вторая картинка — не чужой референс, а наша
    ...
    if laid["outcome"] == PASS:
        ...
        # и это стоило денег: ИЗМЕРЕНО, что все шесть боевых рефок
        ...
        # ровно то, что составитель шаблонов увидел на первом десятисекундном ролике.
        #
...
def stage_style_acceptance(*, styled, style_ref, client_photo, operator_ok_identity=False,
    ...
    if d.get("outcome") == UNMEASURED:
        ...
        if med is None:
            ...
            # допуском оператора, и допуск виден в отчёте. Молча она не проходится никогда.
...
def cut_argv(src, dst, *, first: int, last: int, fps: float, exe: str) -> list:
    ...
    ИЗМЕРЕНО (стенд research-репозитория): `-t` по секундам
...
def stage_kling(*, styled, window, out_path, upload=None, kling=None,
    ...
    # на десятисекундном прогоне стенд напечатал «$0.35», а счёт списал
...
def stage_output_acceptance(*, produced, client_photo, frames_dir,
    ...
    if d.get("outcome") == UNMEASURED:
        ...
        # ТА ЖЕ ЛЕСТНИЦА, ЧТО НА СТИЛИЗОВАННОМ ФОТО. Причина та же и измерена: аксессуар со стилевого референса
        # доезжает до ролика и закрывает лицо на всех кадрах — боевой прогон
        # дал 0.5109 при 0 из 99 в баре, при том что тот же Kling на
            ...
            if r3["outcome"] == PASS:
                ...
                if r4["outcome"] == PASS:
                    ...
                    if r5["outcome"] == PASS:
                        ...
                        # физически не судья, и решением составителя шаблонов от это
...
def main(argv=None) -> int:
    ...
    # распаковщик в проекте был бы вторым способом узнать известное.
    # Без этого канала приёмщик честно отвечает «не смогли» по четырём осям
    # из пяти, и стенд встаёт на ступени 1 — ИЗМЕРЕНО двумя прогонами
