...
ТРИ ИСХОДА, и они не сворачиваются друг в друга:
...
нашёл человек глазами. Составитель шаблонов посмотрел GIF и сказал «на некоторых голова не
...
ЧТО ДЕЛАЕТСЯ СО СВЕДЁННЫМ СТЫКОМ — ЦЕНА МОСТА (решение составителя шаблонов,).
...
отработала, и её оценка 2.141 обошла честные 3.283 соседки. Составитель шаблонов увидел этот
...
ОПЕРАТОРА В КОНТУРЕ НЕТ (решение составителя шаблонов, финальный продукт). Пользователь
...
ЦЕНА, ИЗМЕРЕННАЯ ЗДЕСЬ:

    снятие позы   0.0328 с/кадр на 240x426 (demo/bench/driving, 96 кадров)
                  0.0537 с/кадр на 1280x720 (chain_frames, 96 кадров)
                  0.058  с/кадр — замер составителя шаблонов, сошлось с крупным кадром
    пиксели (резы) 0.0012 с/кадр — в 27 раз дешевле позы, потому и первыми
...
синтетических скелетах без модели. Пиксели для поиска резов — через
...
#: ПОЧЕМУ СУСТАВЫ СВОДЯТСЯ СРЕДНИМ, А НЕ ХУДШИМ — ОТРИЦАТЕЛЬНЫЙ РЕЗУЛЬТАТ.
...
#: 40 парам с медианой по всем 361 не измерялось.
HEAD_SCALE_PAIRS = 40
...
#: ЧЕГО ЗАМЕР НЕ ГОВОРИТ: полка снята на ОДНОЙ фикстуре и на одной петле
...
#: позах: новых опросов детектора сверка не делает вовсе.
DUP_PHASES = 8
...
#: 24.0 — середина плато. Глазами это подтверждается: петли 272..312 и
...
#: шкала заметности от составителя шаблонов): самый длинный мост, который прибор считает
...
#: движения.
#:
#: ШКАЛА ЗАМЕТНОСТИ, ПОДАНА СОСТАВИТЕЛЕМ ШАБЛОНОВ: мост 4 кадра = 0.17 с при 24 к/с почти незаметен; мост 13 кадров =
...
#: составитель шаблонов забраковал глазами за «явный стык на голове». Верхний край плато
#: (12) лежит ровно под названной составителем шаблонов границей заметности (13).
BRIDGE_MAX_FRAMES = 8
...
#: `motion.JUMP_MAX` — значением, а не копией числа. Заимствование законно
...
#: (0.031 с/кадр ИЗМЕРЕНО здесь, 0.058 с/кадр ИЗМЕРЕНО составителем шаблонов). Ждать
...
#: ТРИ ИСХОДА У ЧАСТОТЫ ИСТОЧНИКА, и «неизвестна» не сворачивается в наши
...
#: Коды возврата точки входа. Те же, что у `fork_video` и `fork_stand`:
...
# ---------------------------------------------------------------------------
# ТОЧКА ВНЕДРЕНИЯ: единственное место, где модуль трогает детектор поз.
# ---------------------------------------------------------------------------
...
def read_pose(path) -> dict:
    ...
    столько не спрашивали. Один человек и «не считали» — разные ответы.
...
def read_gray(path):
    ...
    Берётся `motion._gray`: уменьшение до квадрата 96 и перевод в серое
...
def read_head(path) -> dict:
    ...
    уже выведена по исходнику вендора, и вторая разъехалась бы с первой.

    Импорт ленивый: `fork_channels` тянет onnxruntime и 350 МБ весов, а весь
    счёт этого модуля обязан проверяться без них.
    """
    try:
...
def head_scale(paths, *, reader=None, pairs=None, cache=None) -> dict:
    ...
    ЧЕГО ЭТОТ ЗАМЕР НЕ ГОВОРИТ: насколько медиана по 40 парам отличается
...
def head_seam(paths, i, j, *, reader=None, cache=None) -> dict:
    ...
    Три исхода: измерено / лица не видно / спросить нечем. Первое судит,
...
def presence(poses, *, people=None, index=None, gap_min=None) -> dict:
    ...
    оператором), и второго способа здесь не заводится. Наше дело —
...
def states(poses) -> list:
    ...
    Приведение берётся у `pose._normalise`: вторая копия этой арифметики
...
def pose_gap(a, b):
    ...
    `pose_delta` было бы копией знания, которую нечем нарушить.
    """
    import numpy as np
...
def length_is_admissible(length, *, fps=None, min_frames=None) -> bool:
    ...
    ОДНО ЗНАНИЕ — ОДНО МЕСТО: и перечисление длин, и перебор пар спрашивают
...
def admissible_lengths(n_frames, *, fps=None, min_frames=None) -> list:
    ...
    из `SECONDS_MAX` того же модуля, а не своё число.
    """
...
def bridge_frames(seam) -> int | None:
    ...
    РЕШЕНИЕ СОСТАВИТЕЛЯ ШАБЛОНОВ, ИЗ КОТОРОГО ВСЁ ЭТО СЛЕДУЕТ: идеальной петли в реальном
    ...
    Своей арифметики единицы здесь нет и быть не должно: приведение стыка
...
def rank_loops(loops) -> list:
    ...
    310..357 дают то же число, 311..357 -> 10.67). Составитель шаблонов увидел этот стык
    ...
    (а) ДОМЕР ПО БЛИЖАЙШЕМУ ИЗМЕРИМОМУ КРАЮ (сдвинуть i или j на 1-3 кадра).
        ...
        выданное за измерение этой, и потому, что у него самого есть
...
def select(cands, *, overlap_max=None, top=None) -> dict:
    ...
    # которую никто не вызывает, — это менять то, чего нельзя наблюдать.
...
def loop_signature(state_at, i, j, *, phases=None) -> list | None:
    ...
    судить о совпадении по половине подписи нельзя.
    """
...
def repeat_plan(length, *, fps=None) -> list:
    ...
    проверяется `framemath.snap_frames`, а не утверждается.
    """
...
def pick_finalists(worthy, paths, *, top=None, overlap_max=None, head=None,
    ...
    видом лучшей (числа — в `rank_loops`). Теперь очередей две:
        ...
        ВТОРОЙ ПРОХОД добирает оставшиеся места отложенными — с пометкой, в
            ...
            «плохо», и петля с непроверенной головой может оказаться
    ...
    ПОРЯДОК РАБОТ ЗДЕСЬ — ЭТО ЦЕНА. Три первые оси стоят миллисекунды и
    ...
    Три исхода у каждой петли: все оси измерены / какая-то ось не
    ...
    for cand in worthy:
        ...
        # пиксельного масштаба был исходом «не смогли» целиком.
        ...
        if head_off:
            ...
            if seam["outcome"] == PASS:
                ...
                # прибором, что глобальный: второй формулы медианы здесь
...
def find_loops(source, *, out_dir=None, fps=None, reader=None, gray=None,
    ...
    # пришлось бы минутами снимать позы.
    ...
    if src.is_dir():
        ...
        if fps is None:
            ...
            # частоте источника; второй раз спрашивать ffprobe незачем.
    ...
    # десятков миллисекунд позы, а отбрасывает целые области кандидатов.
    ...
    def local_scale(i, j):
        ...
        ЧЕМ ПЛАТИТ МАКСИМУМ ПО ОСЯМ, КОГДА ЗНАМЕНАТЕЛИ РАЗНЫЕ (, числом).
        ...
        составитель шаблонов забраковал глазами.

        ЧТО ЛОКАЛЬНЫЙ ШАГ НЕ ОБЪЯСНИЛ, И ЭТО НАДО СКАЗАТЬ ВСЛУХ. На стыке
    ...
    # рядом с вердиктом — сколько посчитано, сколько отвергнуто и сколько
