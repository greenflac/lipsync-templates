...
негодным был не он, а якорь. Замерено на калибровочном наборе и записано в
его манифест:
...
ЧЕТЫРЕ ЧИСЛА, И ОДНО ИЗ НИХ — НЕГАТИВНЫЙ КОНТРОЛЬ.
...
Закрыто решением составителя шаблонов (ХЭНДОФ §2, §6 A): **референс = сырая фотография
...
трёх, и это записано числом, а не сглажено. Машиночитаемо — в
`ACCEPTANCE_ROWS` ниже, чтобы отчёт брался из одного места, а не
...
исключена из набора составителем шаблонов продукта, и манифест это прямо говорит —
...
#: Три исхода вместо двух. «Не смогли» не сворачивается ни в один из двух.
...
#: НЕ СВОЯ константа: берётся у прибора, потому что это его свойство.
from .identity_arcface import MIN_COVERAGE  # noqa: E402

#: Кадр заведомо ЧУЖОГО человека. Одно место на весь проект: и негативный
...
#: брался отсюда, а не пересказывался прозой, которая разъезжается.
#: `reproduced=None` значит «не смогли», и это ТРЕТИЙ ИСХОД, а не провал.
ACCEPTANCE_ROWS = {
...
def axis(frames, *, raw_photo: str | Path,
    ...
    """
    # ОБА якоря проверяются ДО первого обращения к прибору. Сначала здесь
...
def before_after_restore(before_frames, after_frames, *,
    ...
    составителем шаблонов, и у неё измеренное последствие: лицо выходит 63–80 px против
...
def acceptance_report() -> dict:
    ...
    агрегатный `true` читается как полная работа, поэтому здесь «1 из 3»
    и поимённо, какая именно. строки, которые не смогли, считаются
    отдельно от строк, которые провалились, — иначе «ноль провалов» при двух
    непроведённых замерах прочтётся как успех.

    Источник — `ACCEPTANCE_ROWS`, одно место. Функция ничего не измеряет
    ...
    return {
        ...
        "note": (f"приёмка §6 A: воспроизведено {len(done)} строк(и) из "
                 ...
                 # — и тот, кто это прочтёт, поверит тексту, а не числам.
